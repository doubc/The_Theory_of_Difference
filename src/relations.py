...
def qualitative_judgment(
    ...
    if "breakout" in tendency:
        # 结合 flux 和方向判断
        # 语义说明（v4.0）：
        #   breakout = 价格正在突破 zone（客观状态）
        #   突破失败 = 突破后无法站稳，很快回归 zone 内（含假突破）
        #   趋势上行/下行 = 突破后站稳，趋势延续
...
def build_system_state(
    ...
    ⚠️ 副作用（enrichment 模式）：此函数会原地修改 Structure 对象的以下字段：
      - s.motion (如果为 None)
    ...
    调用方应传入刚创建的 Structure（如 compiler/pipeline.py 中的用法），
    不要传入已被其他逻辑共享引用的 Structure 对象。
    """
