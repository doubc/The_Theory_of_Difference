...
class StructureGraph:
    ...
    # ─── 下游查询接口（quality / signals 可用）─────────────────

    def get_zone_history(self, symbol: str, price_center: float, tolerance: float = 0.02) -> dict:
        """
        查询某品种某价位的历史 Zone 信息。

        用于 quality.py 的 Zone 强度增强：
        历史上被更多结构围绕的 Zone，其支撑/阻力更可靠。

        Args:
            symbol: 品种代码
            price_center: Zone 中心价格
            tolerance: 价格匹配容差（相对比例）

        Returns:
            {
                "zone_key": str,
                "structure_count": int,      # 历史上该 Zone 出现的结构数
                "avg_cycle_count": float,    # 平均 cycle 数
                "avg_flux": float,           # 平均通量
                "narratives": list[str],     # 历史叙事列表
                "evolution_depth": int,      # 最长演化链深度
            }
        """
        # 找到匹配的 Zone 节点
        matching_zone = None
        for node in self.G.nodes:
            if self.G.nodes[node].get("node_type") != NodeType.ZONE.value:
                continue
            center = self.G.nodes[node].get("price_center", 0)
            if center > 0 and abs(center - price_center) / center <= tolerance:
                matching_zone = node
                break

        if matching_zone is None:
            return {"zone_key": "", "structure_count": 0, "avg_cycle_count": 0,
                    "avg_flux": 0, "narratives": [], "evolution_depth": 0}

        # 找到所有属于该 Zone 的结构
        struct_nodes = [
            n for n in self.G.predecessors(matching_zone)
            if (self.G.nodes[n].get("node_type") == NodeType.STRUCTURE.value
                and self.G[n][matching_zone].get("edge_type") == EdgeType.BELONGS_TO.value)
        ]
        ...
        for sn in struct_nodes:
            cc = self.G.nodes[sn].get("cycle_count", 0)
            if cc:
                cycle_counts.append(cc)
            flux = self.G.nodes[sn].get("conservation_flux", 0)
            if flux:
                fluxes.append(flux)
            narr = self.G.nodes[sn].get("narrative", "")
            if narr:
                narratives.append(narr)
            chain = self.get_structure_evolution_chain(sn)
            max_depth = max(max_depth, len(chain))

        return {
            "zone_key": matching_zone,
            "structure_count": len(struct_nodes),
            "avg_cycle_count": sum(cycle_counts) / len(cycle_counts) if cycle_counts else 0,
            "avg_flux": sum(fluxes) / len(fluxes) if fluxes else 0,
            "narratives": narratives,
            "evolution_depth": max_depth,
        }

    def get_symbol_zone_count(self, symbol: str) -> dict[str, int]:
        """
        获取某品种所有 Zone 的结构计数。

        Returns:
            {zone_key: structure_count}
        """
        zone_counts: dict[str, int] = {}
        for node in self.G.nodes:
            if self.G.nodes[node].get("node_type") != NodeType.STRUCTURE.value:
                continue
            sym = self.G.nodes[node].get("symbol", "")
            if sym != symbol:
                continue
            zk = self.G.nodes[node].get("zone_key", "")
            if zk:
                zone_counts[zk] = zone_counts.get(zk, 0) + 1
        return zone_counts

    def get_contrast_distribution(self, symbol: str) -> dict[str, int]:
        """
        获取某品种的反差类型分布。
        ...
        for node in self.G.nodes:
            ...
            if sym != symbol:
                continue
            ct = self.G.nodes[node].get("contrast_type", "unknown")
