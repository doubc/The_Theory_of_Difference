...
P1.1 升级（2026-05-01）：
- RankScore 扩展：新增 geometric/relational/motion/family 四个细分维度
- combine_scores() 签名扩展：接收完整的相似度分解，不再只收一个 float
- 100% 向后兼容：不传细分维度 = 旧行为（base_score 兜底）

设计原则：
1. 与 engine.py 解耦 — ranker 不知道 RetrievalEngine 的存在
2. 纯函数 — combine_scores() 无副作用
3. 可配置 — 权重从配置文件读取，不硬编码
4. 信息守恒 — 相似度的多维分解不应在排序环节丢失
"""
...
class RankScore:
    ...
    P1.1: 新增四个细分维度，与 SimilarityScore 的六维对齐：
    - base_score: 基础相似度（similarity.total，兜底权重）
    - geometric: 几何相似度
    - relational: 关系相似度
    - motion: 运动相似度
    - family: 结构族相似度
...
# 细分维度默认 0 = 不影响旧行为，由 base_score 兜底
...
def combine_scores(
    ...
    P1.1: 新增 geometric/relational/motion/family 四个细分维度参数。
    当细分维度权重全为 0（默认）时，退化为旧行为（只用 base_score）。
    当细分维度有权重时，细分维度参与计算，base 权重保留但不覆盖细分信息。

    Args:
        base_score: 基础相似度（similarity.total，兜底权重）
        geometric: 几何相似度 [0, 1]
        relational: 关系相似度 [0, 1]
        motion: 运动相似度 [0, 1]
        family: 结构族相似度 [0, 1]
        graph_score: 图谱上下文得分
    ...
    w_geo = w.get("geometric", 0.0)
    w_rel = w.get("relational", 0.0)
    w_mot = w.get("motion", 0.0)
    w_fam = w.get("family", 0.0)
    w_graph = w.get("graph", 0.12)
    w_recency = w.get("recency", 0.05)
    w_quality = w.get("quality", 0.05)

    # 归一化权重（确保总和为 1）
    w_sum = w_base + w_geo + w_rel + w_mot + w_fam + w_graph + w_recency + w_quality
    if w_sum > 0:
        w_base /= w_sum
        w_geo /= w_sum
        w_rel /= w_sum
