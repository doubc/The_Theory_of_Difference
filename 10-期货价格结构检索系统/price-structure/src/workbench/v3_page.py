...
with sub_tabs[0]:
    ...
    if q_run and q_symbol:
        ...
        with st.spinner(f"编译 {q_symbol}..."):
            bars = fetch_bars(q_symbol, freq="1d", timeout=15)
            if bars:
...
with sub_tabs[1]:
    ...
    if r_run:
        from src.data.sina_fetcher import available_contracts, fetch_bars
        from src.compiler.pipeline import compile_full, CompilerConfig
        from src.resonance import ResonanceDetector

        all_contracts = []
        for group, codes in available_contracts().items():
            all_contracts.extend(codes[:5])  # 每组取前5个，避免太慢
        ...
        for i, code in enumerate(all_contracts):
            try:
                bars = fetch_bars(code, freq="1d", timeout=10)
        ...
        if lifecycles:
            ...
            if st.button(f"📝 立即记录 {l_symbol.upper()}", key="l_record"):
                ...
                with st.spinner(f"编译 {l_symbol}..."):
                    bars = fetch_bars(l_symbol, freq="1d", timeout=15)
                    if bars:
...
with sub_tabs[3]:
    ...
    if i_run and i_symbol:
        ...
        with st.spinner(f"拉取 {i_symbol} 5分钟数据..."):
            bars_5m = fetch_bars(i_symbol, freq="5m", timeout=15)
