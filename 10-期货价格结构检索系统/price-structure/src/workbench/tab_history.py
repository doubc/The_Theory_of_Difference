        ...
        if run_search:
            ...
            for si, sym in enumerate(search_symbols):
                ...
                for hs in sym_result.structures:
                    ...
                    direction, move, days = "unclear", 0.0, 0
                    if hs.t_end:
                        ...
                        if future:
                            ...
                            if start_p > 0:
                                peak_price, peak_idx = start_p, 0
                                trough_price, trough_idx = start_p, 0
                                for bi, b in enumerate(future):
                                    if b.high > peak_price:
                                        peak_price, peak_idx = b.high, bi
                                    if b.low < trough_price:
                                        trough_price, trough_idx = b.low, bi
                                up = (peak_price - start_p) / start_p
                                down = (start_p - trough_price) / start_p
                                if up >= down:
                                    direction, move, days = "up", up, peak_idx
                                else:
                                    direction, move, days = "down", down, trough_idx
