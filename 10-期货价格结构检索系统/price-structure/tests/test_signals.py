...
from src.models import (
    ...
    SignalKind, FakeBreakoutPattern, SystemState, Signal
)
...
# ============================================================
# Test Case 7: New Fake Breakout Patterns (WICK_CLUSTER, TIME_TRAP)
# ============================================================

def test_new_fake_patterns():
    """Test new fake breakout patterns"""
    print("\n" + "="*60)
    print("Test 7: New Fake Breakout Patterns (WICK_CLUSTER, TIME_TRAP)")
    print("="*60)

    structure = create_structure_with_zone(center=100.0, bandwidth=2.0, n_cycles=5)
    base_date = datetime(2024, 1, 1)

    # Test 7.1: FAKE_WICK_CLUSTER (连续影线簇)
    print("\n--- Test 7.1: FAKE_WICK_CLUSTER ---")
    # 3根K线: 实体在Zone内(98-102), 但high都>102
    bars = create_bars_sequence(base_date, 17, base_price=100.0)
    for i in range(3):
        bars.append(create_bar(
            base_date + timedelta(days=17 + i),
            open_price=100.0,
            high=103.5,    # 上影线探出Zone上沿102
            low=99.5,
            close=100.5,   # 实体在Zone内
            volume=1200
        ))
    # flux negative (opposite to upward wicks)
    ss = create_system_state(flux=-0.5, structural_age=10)

    is_fake, pattern, conf = detect_fake_breakout(structure, bars, ss)
    print(f"  Is fake: {is_fake}, Pattern: {pattern}, Conf: {conf:.2f}")
    if is_fake and pattern == FakeBreakoutPattern.FAKE_WICK_CLUSTER:
        print(f"  [PASS] Correctly identified FAKE_WICK_CLUSTER")
    else:
        print(f"  [WARN] FAKE_WICK_CLUSTER not identified (got {pattern})")

    # Test 7.2: FAKE_TIME_TRAP (时间陷阱)
    print("\n--- Test 7.2: FAKE_TIME_TRAP ---")
    # Zone外停留3天, 然后回归
    bars = create_bars_sequence(base_date, 16, base_price=100.0)
    # 3天在Zone上方
    for i in range(3):
        bars.append(create_bar(
            base_date + timedelta(days=16 + i),
            open_price=103.0,
            high=104.0,
            low=102.5,
            close=103.0,   # close > upper(102)
            volume=1000 - i * 100  # 量能递减
        ))
    # 今天回到Zone内
    bars.append(create_bar(
        base_date + timedelta(days=19),
    ...
    ss = create_system_state(flux=-0.4, structural_age=10)

    is_fake, pattern, conf = detect_fake_breakout(structure, bars, ss)
    print(f"  Is fake: {is_fake}, Pattern: {pattern}, Conf: {conf:.2f}")
    if is_fake and pattern == FakeBreakoutPattern.FAKE_TIME_TRAP:
        print(f"  [PASS] Correctly identified FAKE_TIME_TRAP")
    else:
        print(f"  [INFO] FAKE_TIME_TRAP detection: is_fake={is_fake}, pattern={pattern}")


# ============================================================
# Test Case 8: ATR-based Stop Loss
# ============================================================

def test_atr_stop_loss():
    """Test ATR-based stop loss calculation"""
    print("\n" + "="*60)
    print("Test 8: ATR-based Stop Loss")
    print("="*60)

    from src.signals import compute_atr, _compute_risk_reward

    base_date = datetime(2024, 1, 1)
    bars = create_bars_sequence(base_date, 20, base_price=100.0, volume_base=1000)

    # Test 8.1: ATR computation
    print("\n--- Test 8.1: ATR Computation ---")
    atr = compute_atr(bars, period=14)
    print(f"  ATR(14) = {atr:.4f}")
    assert atr > 0, "ATR should be positive"
    print(f"  [PASS] ATR is positive")

    # Test 8.2: Stop loss with ATR vs without
    print("\n--- Test 8.2: Stop Loss Comparison ---")
    sl_atr, tp_atr, rr_atr = _compute_risk_reward(
        "long", 100.0, 102.0, 98.0, 2.0, atr=atr, quality_tier="B"
    )
    sl_bw, tp_bw, rr_bw = _compute_risk_reward(
        "long", 100.0, 102.0, 98.0, 2.0, atr=0.0, quality_tier="B"
    )
    print(f"  With ATR:    SL={sl_atr:.2f}, TP={tp_atr:.2f}, RR={rr_atr:.2f}")
    print(f"  Without ATR: SL={sl_bw:.2f}, TP={tp_bw:.2f}, RR={rr_bw:.2f}")
    assert sl_atr != sl_bw, "ATR-based and bandwidth-based stop loss should differ"
    print(f"  [PASS] ATR produces different stop loss than bandwidth fallback")

    # Test 8.3: Different quality tiers → different stop loss widths
    print("\n--- Test 8.3: Quality Tier Affects Stop Width ---")
    sl_a, _, _ = _compute_risk_reward("long", 100.0, 102.0, 98.0, 2.0, atr=atr, quality_tier="A")
    sl_c, _, _ = _compute_risk_reward("long", 100.0, 102.0, 98.0, 2.0, atr=atr, quality_tier="C")
    print(f"  Tier A SL={sl_a:.2f} (tighter), Tier C SL={sl_c:.2f} (wider)")
    assert sl_a > sl_c, "Tier A should have tighter stop (higher SL for long)"
    print(f"  [PASS] Tier A stop is tighter than Tier C")


# ============================================================
# Test Case 9: Entry Tolerance
# ============================================================

def test_entry_tolerance():
    """Test entry price tolerance for Zone boundary entries"""
    print("\n" + "="*60)
    print("Test 9: Entry Tolerance")
    print("="*60)
    ...
    signal = generate_signal(structure, bars, ss)
    if signal and signal.kind == SignalKind.FAKE_BREAKOUT:
        print(f"  Entry price: {signal.entry_price}")
        print(f"  Entry limit upper: {signal.entry_limit_upper}")
        print(f"  Entry limit lower: {signal.entry_limit_lower}")

        if signal.direction == "long":
            assert signal.entry_price == 98.0, f"Long fake breakout entry should be Zone lower (98), got {signal.entry_price}"
            assert signal.entry_limit_upper > signal.entry_price, "Limit upper should be above entry"
            print(f"  [PASS] Long entry at Zone lower with tolerance upper={signal.entry_limit_upper:.1f}")
        elif signal.direction == "short":
            assert signal.entry_price == 102.0, f"Short fake breakout entry should be Zone upper (102), got {signal.entry_price}"
            assert signal.entry_limit_lower < signal.entry_price, "Limit lower should be below entry"
            print(f"  [PASS] Short entry at Zone upper with tolerance lower={signal.entry_limit_lower:.1f}")
    else:
        print(f"  [WARN] No fake breakout signal generated")


# ============================================================
# Test Case 10: Timeframe & TTL
# ============================================================

def test_timeframe_ttl():
    """Test timeframe and TTL fields"""
    print("\n" + "="*60)
    print("Test 10: Timeframe & TTL")
    print("="*60)
    ...
    signal = generate_signal(structure, bars, ss, timeframe="60min")
    if signal:
        print(f"  Timeframe: '{signal.timeframe}'")
        print(f"  TTL bars: {signal.ttl_bars}")
        print(f"  Signal bars index: {signal.signal_bars_index}")
        print(f"  ATR value: {signal.atr_value:.4f}")

        assert signal.timeframe == "60min", f"Expected '60min', got '{signal.timeframe}'"
        assert signal.ttl_bars > 0, "TTL should be positive for non-expired signals"
        assert signal.atr_value > 0, "ATR should be computed"
        print(f"  [PASS] Timeframe, TTL, and ATR all set correctly")

        # Test serialization round-trip
        d = signal.to_dict()
        assert d["timeframe"] == "60min"
        assert d["ttl_bars"] > 0
        assert d["atr_value"] > 0
        signal2 = Signal.from_dict(d)
        assert signal2.timeframe == "60min"
        assert signal2.ttl_bars == signal.ttl_bars
        print(f"  [PASS] Serialization round-trip preserves new fields")
    else:
        print(f"  [WARN] No signal generated")


# ============================================================
# Test Case 11: Trade Plan Format
# ============================================================

def test_trade_plan_format():
    """Test enriched trade plan output format"""
    print("\n" + "="*60)
    print("Test 11: Trade Plan Format")
    print("="*60)
    ...
    signal = generate_signal(structure, bars, ss, timeframe="60min")
    if signal:
        plan = generate_trade_plan(signal, structure, symbol="CU000", current_price=103.5)
        print(f"  Trade plan:\n{plan}")

        # Verify enriched format
        lines = plan.split("\n")
        assert len(lines) >= 3, f"Expected >= 3 lines, got {len(lines)}"
        assert "60min" in plan, "Should contain timeframe"
        assert "CU000" in plan, "Should contain symbol"
        assert "当前" in plan, "Should contain current price"
        assert "有效期" in plan, "Should contain TTL"
        assert "ATR" in plan, "Should contain ATR"
        assert "通量" in plan, "Should contain flux status"
        print(f"  [PASS] Trade plan has all expected elements")

        # Test neutral signal plan
        ...
        neutral_plan = generate_trade_plan(neutral_sig, structure, symbol="CU000")
        print(f"\n  Neutral plan: {neutral_plan}")
        assert "观望" in neutral_plan or "结构老化" in neutral_plan
        print(f"  [PASS] Neutral signal plan renders correctly")
    else:
        print(f"  [WARN] No signal generated")
...
def run_all_tests():
    ...
    # ── New tests for v4.1 features ──

    try:
        results.append(("New Fake Patterns (WICK/TIME)", test_new_fake_patterns()))
    except Exception as e:
        print(f"\n[FAIL] New Fake Patterns test failed: {e}")
        import traceback
        traceback.print_exc()
        results.append(("New Fake Patterns (WICK/TIME)", False))

    try:
        results.append(("ATR Stop Loss", test_atr_stop_loss()))
    except Exception as e:
        print(f"\n[FAIL] ATR Stop Loss test failed: {e}")
        import traceback
        traceback.print_exc()
        results.append(("ATR Stop Loss", False))

    try:
        results.append(("Entry Tolerance", test_entry_tolerance()))
    except Exception as e:
        print(f"\n[FAIL] Entry Tolerance test failed: {e}")
        import traceback
        traceback.print_exc()
        results.append(("Entry Tolerance", False))

    try:
        results.append(("Timeframe & TTL", test_timeframe_ttl()))
    except Exception as e:
        print(f"\n[FAIL] Timeframe & TTL test failed: {e}")
        import traceback
        traceback.print_exc()
        results.append(("Timeframe & TTL", False))

    try:
        results.append(("Trade Plan Format", test_trade_plan_format()))
    except Exception as e:
        print(f"\n[FAIL] Trade Plan Format test failed: {e}")
        import traceback
        traceback.print_exc()
        results.append(("Trade Plan Format", False))
