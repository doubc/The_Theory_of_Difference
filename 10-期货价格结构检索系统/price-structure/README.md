...
- K 线图 + Zone 标注
- 结构卡片：Zone、运动标签、守恒通量、高压缩警告、**价格 vs Zone 位置**
- 按运动态分组：正在破缺 → 趋向确认 → 形成中
- 💡 **研究建议**：每张卡片可展开查看下一步研究动作
- ⚖️ **风控评级**：基于关注度评分给出高/中/低关注度 + 建议关注比例
- 🔗 **跨品种信号一致性**：按交易所分组，判断板块方向是否一致
- 📋 **每日变化报告**：自动对比上次扫描，显示新增/变化/退出的结构
...
输入任意合约代码，从新浪实时拉取数据 → 编译结构 → 展示分析：

- 预置 65 个合约（国内期货/外盘/外汇）+ 自由输入任意代码（如 cu2507、rb2510）
- 自动识别数据源：🇨🇳 国内 / 🌍 外盘 / 💱 外汇
- 支持日线 + 5 分钟线
- 结构卡片 + K 线图 + 不变量详情
- 数据采集器：`src/data/sina_fetcher.py`（从新浪 API 抓取，不依赖外部数据库）
