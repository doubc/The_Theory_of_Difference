...
### 活动日志 — `src/workbench/activity_log.py`

**问题**：扫描、检索、对照的结果用完就丢，没有形成积累

**新增 `ActivityLog` 类**（JSONL 格式）：
- `save_scan` — 全市场扫描结果自动保存
- `save_retrieval` — 历史检索结果自动保存
- `save_compare` — 跨品种对比结果自动保存
- `save_contract` — 合约检索结果自动保存
- `save_insight` — 人工洞察记录
- `search` — 按品种/类型/时间/标签筛选
- `get_stats` — 活动统计（按类型/品种/日期）
- `get_timeline` — 品种活动时间线

**自动接入**：
- `tab_scan.py` — 扫描完成后自动保存
- `tab_history.py` — 检索完成后自动保存
- `tab_contract.py` — 合约编译后自动保存

**复盘日志升级** (`tab_journal.py`)：
- 新增「📊 活动日志」子标签页
- 统计概要（总记录/扫描/检索/合约）
- 按类型/品种/时间筛选
- 详情展开（按类型展示：扫描Top10/检索案例/合约结构）
- 活动趋势柱状图（最近14天）
