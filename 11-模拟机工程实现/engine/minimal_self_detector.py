...
核心区分：
- 功能（function）：局部关系对整体延续的不对称贡献（组织内部的不对称）
- 最小自我（minimal self）：这种不对称性被结构自身追踪和响应（不对称性的自我参照）
...
工程指标：
- 最小自我指数（MSI）∈ [0, 1]：三条件的综合度量
- 视角不对称度 = 各部分敏感度的基尼系数
- 历史依赖度 = 相同输入在不同历史下的响应差异
- 自我参照度 = 响应对后续基线的影响强度

MSI 与 ODI 的关系：
- MSI 应在 ODI > 0.5（前主体态地板）之后才开始增长
- ODI < 0.5 时，MSI 恒为 0（结构尚未准备好）
- ODI → 1.0 时，MSI 可能趋于饱和（结构完全自我追踪）
