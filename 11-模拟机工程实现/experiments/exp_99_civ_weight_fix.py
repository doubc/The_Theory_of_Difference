"""experiments/exp_99_civ_weight_fix.py

Phase 4 Experiment 9: CIV Weight Fix Validation

Purpose: Validate that removing CIV from multi-signal turning point detection
breaks the NSE-CIV positive feedback loop identified in exp_98.

exp_98 finding: CIV weight (0.2) in turning point detector creates a direct
positive feedback channel:
  More turning points -> more self-history -> more narrative activity
  -> more CIV events -> stronger CIV signal -> more turning points

This caused CIV explosion in 2/8 seeds (CIV=184, 182) while AMC remained
inactive (momentum_bonus ~0.485 constant across all seeds).

Fix: Set CIV weight to 0.0 in turning point detection signal vector.
This breaks the feedback loop at its source.

Config: Same as exp_98 (N0=72, steps=1600, 8 seeds)
...
Hypotheses:
  ...
  H5: CIV mean in [3, 15] (should stabilize without CIV feedback loop)
  H6: min CIV >= 3

Key difference from exp_98: CIV weight = 0.0 (was 0.2)
...
def run_single_seed(N0=72, steps=1600, seed=142, sample_interval=10,
    ...
    if use_nse:
        ...
        # KEY FIX: CIV weight already 0.0 in defaults after code change
...
def main():
    ...
    output = {
        ...
        'config': {
            ...
            'amc': 'ON (adaptive momentum control)',
            'ilp': 'ON (institutional layer protection)',
            'key_fix': 'CIV weight 0.2 -> 0.0 (breaks NSE-CIV feedback loop)',
        },
    ...
    output_path = os.path.join(output_dir, f'exp_99_results_{timestamp}.json')
