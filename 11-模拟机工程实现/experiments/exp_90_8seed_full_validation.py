"""experiments/exp_90_8seed_full_validation.py

Phase 3 Experiment 18: 8-Seed Full Validation with Best Configuration

Best config from exp_87/88/89:
  - momentum_bonus=0.3, momentum_decay=0.95
  - coherence_threshold=0.5
  - gbc_soft_nudge=0.2
  - N0=72, steps=1600

Seeds: [42, 142, 242, 342, 442, 542, 642, 742] (all 8 seeds from exp_87)

This is the definitive Phase 3 validation run. If H1/H2/H3 pass with 8 seeds,
Phase 3 (Pre-subjective -> Phenomenal Consciousness) is validated.

Hypotheses:
  H1: Mean CIV >= 5.0 (robust across 8 seeds)
  H2: GBC coherence mean >= 0.55
  H3: GBC pass_rate >= 0.3
  H4: All seeds CIV >= 3 (no catastrophic failures)
"""
...
def main():
    print("=" * 70)
    print("exp_90: 8-Seed Full Validation (bonus=0.3, nudge=0.2, threshold=0.5)")
    print("=" * 70)
    ...
    # Per-seed comparison with exp_87 (baseline)
    exp_87_civ = {42: 10, 142: 10, 242: 10, 342: 9, 442: 10, 542: 3, 642: 4, 742: 2}
    print(f"\n  {'Seed':>6} | {'CIV':>5} | {'exp_87':>7} | {'Delta':>6} | {'GBC_coh':>8} | {'GBC_pass':>9} | Status", flush=True)
    print(f"  {'-'*6}-+-{'-'*5}-+-{'-'*7}-+-{'-'*6}-+-{'-'*8}-+-{'-'*9}-+-{'-'*6}", flush=True)
    for r in all_results:
        status = "PASS" if r['civilization_steps'] >= 5 else "FAIL"
        delta = r['civilization_steps'] - exp_87_civ.get(r['seed'], 0)
        delta_str = f"+{delta}" if delta >= 0 else str(delta)
        print(f"  {r['seed']:>6} | {r['civilization_steps']:>5} | {exp_87_civ.get(r['seed'], '?'):>7} | {delta_str:>6} | {r['gbc']['coherence_mean']:>8.4f} | {r['gbc']['pass_rate']:>9.4f} | {status}", flush=True)
    ...
    for mech in sorted(all_mechanisms):
        ...
        print(f"    {mech:>20}: mean={float(np.mean(vals)):+.4f}, min={float(np.min(vals)):+.4f}, max={float(np.max(vals)):+.4f}", flush=True)
    ...
    print(f"\n  *** PHASE 3 VALIDATION: {'ALL HYPOTHESES PASSED' if all_pass else 'SOME HYPOTHESES FAILED'} ***", flush=True)

    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                f'exp_90_results_{ts}.json')
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump({
            'experiment': 'exp_90_8seed_full_validation',
            'timestamp': ts,
            'seeds': seeds,
            'n_seeds': len(seeds),
            'config': {
