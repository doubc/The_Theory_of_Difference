"""experiments/exp_97_multisignal_h4_validation.py

Phase 4 Experiment 7: Multi-Signal H4 Validation

Purpose: Validate the multi-signal turning point detection fix for H4.
...
The fix (commit 0c4f264): SelfHistoryAccumulator now uses weighted multi-signal
second derivative detection: MSI(0.4) + ODI(0.3) + CIV(0.2) + GBC(0.1).
Even when MSI is flat, ODI/CIV/GBC may still have inflection points.

Config: Same as exp_96 (N0=72, steps=1600, 8 seeds)
    CSC: ON (exp_95 stable config)
    GBC: ON (random direction init, soft nudge=0.2)
    ILP: OFF
    AMC: OFF
    NSE: ON (with multi-signal turning point detection enabled)
...
Hypotheses:
  ...
  H4: Self history depth > 0.05 (multi-signal turning points detected)
  H5: CIV mean in [3, 15] (baseline stability preserved)
  H6: min CIV >= 3 (no seed collapses)

Key difference from exp_96: NSE uses multi-signal turning point detection
(MSI + ODI + CIV + GBC) instead of MSI-only. This should fix H4 while
preserving H1/H2/H3/H5/H6 results.
"""
...
def run_single_seed(N0=72, steps=1600, seed=142, sample_interval=10,
    ...
    if use_nse:
        ...
        # Ensure multi-signal is enabled (default=True, but explicit for clarity)
    ...
    print(f"  [seed={seed}] Running (CSC={csc_label}, NSE={nse_label}, multi-signal=ON)...", flush=True)
    start = time.time()
    result = evolver.run()
...
def evaluate_hypotheses(results):
    ...
    # CIV mean = total civ_count / n_seeds (approximation from sampled steps)
    ...
    # H4: history depth > 0.05 OR turning points > 0 (multi-signal should help)
...
def main():
    ...
    print(f"exp_97: Multi-Signal H4 Validation")
    print(f"  N0={N0}, steps={STEPS}, {len(SEEDS)} seeds")
    print(f"  CSC=ON, GBC=ON (random dir), NSE=ON (multi-signal=ON)")
    print(f"  ILP=OFF, AMC=OFF")
    print()
    ...
    output_path = os.path.join(output_dir, f'exp_97_results_{timestamp}.json')
