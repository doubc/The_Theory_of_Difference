"""experiments/exp_80_extended_evolution.py
...
def run_experiment(N0=96, steps=1600, seed=142, sample_interval=10):
    ...
    print(f"[exp_80] N0={N0}, steps={steps}, seed={seed}, sample_interval={sample_interval}")
    start = time.time()
    result = evolver.run()
    elapsed = time.time() - start
    print(f"[exp_80] Completed in {elapsed:.1f}s")
...
def main():
    ...
    output_file = os.path.join(
        ...
        f'exp_80_results_{timestamp}.json'
    )

    print(f"{'='*60}")
    print(f"exp_80: Narrative Activation Verification")
    ...
    all_results = []
    for seed in [142]:
        run_data = run_experiment(N0=96, steps=1600, seed=seed, sample_interval=10)
        all_results.append(run_data)
    diag = all_results[0]['diagnostic']

    # Print summary
    print(f"\n{'='*60}")
    print(f"exp_80 Results (seed={diag['seed']}):")
    ...
    # Save
    summary = {
        'experiment': 'exp_80_extended_evolution',
        'timestamp': timestamp,
        'description': 'exp_80: verifier post_odi fix + seed reproducibility (142, 242)',
        'fixes_tested': [
