"""experiments/exp_95_random_direction_validation.py

Phase 4 Experiment 5: Random Direction Initialization Validation

Purpose: Validate that random 50/50 direction initialization balances GBC constraint
strength, avoiding both the under-constraint (exp_93: CIV=48.5) and over-constraint
(exp_94: CIV=2.0) extremes.

Background:
- exp_93: direction=all_zeros → GBC inactive → CIV exploded to 48.5
- exp_94: direction=all_ones → GBC over-constrained → CIV collapsed to 2.0
- Fix (907fbcd): direction=random(50/50 +1/-1) → expected self_sustaining≈0.5

Config: Same as exp_93/exp_94 (N0=72, steps=1600, 8 seeds)
    CSC: ON (same config)
    ILP: OFF
    AMC: OFF
    GBC soft nudge: 0.2
...
Hypotheses:
  H1: GBC n_checks > 0 (GBC executes)
  H2: GBC coherence mean in [0.3, 0.7] (moderate coherence)
  H3: CIV mean in [3, 15] (balanced, near exp_90 baseline of 5.25)
  H4: min CIV >= 3 (no seed collapses)
  H5: CSC CSCI mean >= 0.3 (CSC still works)
"""
...
def main():
    ...
    print("=" * 70)
    print("exp_95: Random Direction Initialization Validation")
    print("=" * 70)
    print(f"Seeds: {seeds}")
    print(f"N0={N0}, steps={steps}")
    print(f"CSC: ON, ILP: OFF, AMC: OFF")
    ...
    h2_pass = 0.3 <= np.mean(gbc_coh) <= 0.7
    ...
    print(f"H2 (GBC coh in [0.3,0.7]):   {'PASS' if h2_pass else 'FAIL'} ({np.mean(gbc_coh):.4f})")
    ...
    print("\n--- vs exp_93, exp_94, exp_90 ---")
    print(f"CIV mean:    {np.mean(civ_values):.2f} (exp_94: 2.00, exp_93: 48.50, exp_90: 5.25)")
    print(f"CIV min:     {min(civ_values)} (exp_94: 1, exp_93: 4, exp_90: 2)")
    print(f"GBC n_chk:   {np.mean(gbc_n):.0f} (exp_94: 16, exp_93: 0)")
    print(f"GBC coh:     {np.mean(gbc_coh):.4f} (exp_94: 0.5671, exp_93: 0.0000)")

    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    output_path = os.path.join(
        PROJECT_ROOT, 'experiments', f'exp_95_results_{timestamp}.json'
    )
    output = {
        'experiment': 'exp_95_random_direction_validation',
        'timestamp': timestamp,
        'seeds': seeds,
        'n_seeds': len(seeds),
        'config': {
