"""experiments/exp_100_civ_rate_limiter.py

Phase 4 Experiment 10: CIV Rate Limiter Validation

Purpose: Validate that CIVRateLimiter — a structural rate limiter on
CIVILIZATION-level narrative generation — fixes the H5 failure.

exp_99 finding: Removing CIV from turning point detection did NOT fix CIV explosion.
The primary driver is the narrative recursion operator generating CIVILIZATION-level
events at a seed-dependent rate. Seeds 42/142 generated CIV at ~0.78/step.

Fix: CIVRateLimiter in NarrativeRecursionOperator caps CIV generation rate.
When sliding-window CIV rate > 0.1/step, subsequent CIVILIZATION narratives
are downgraded to INSTITUTIONAL. This is a pure structural mechanism — no
optimization, no value judgment.

Config: Same as exp_99 (N0=72, steps=1600, 8 seeds)
    ...
    CIVRateLimiter: ON (window=50, max_rate=0.1, cooldown=20)
...
Hypotheses:
  ...
  H5: CIV mean in [3, 15] (should stabilize with rate limiter)
  H6: min CIV >= 3

Key difference from exp_99: CIVRateLimiter active (downgrades excess CIV to INSTITUTIONAL)
Expected: H5 should pass (CIV stabilized), H1-H4/H6 should remain stable.
"""
...
class MomentumNarrativeOperatorV3(NarrativeRecursionOperator):
    def __init__(self, bias_dimension=128, filter_magnitude_threshold=0.02,
        ...
        # CIVRateLimiter is inherited from NarrativeRecursionOperator.__init__
        # but since we're overriding __init__, we need to add it:
        from models.narrative_self import CIVRateLimiter
...
def main():
    ...
    print(f"exp_100: CIV Rate Limiter Validation")
    print(f"  N0={N0}, steps={STEPS}, {len(SEEDS)} seeds")
    print(f"  CSC=ON, GBC=ON (random dir), NSE=ON (CIV weight=0.0)")
    print(f"  AMC=ON, ILP=ON")
    print(f"  CIVRateLimiter=ON (window=50, max_rate=0.1, cooldown=20)")
    print(f"  Key fix: rate limiter downgrades excess CIV to INSTITUTIONAL")
    print()
    ...
    print()
    print("CIV RATE LIMITER SUMMARY")
    print("-" * 40)
    ...
    output = {
        ...
        'config': {
            ...
            'amc': 'ON (adaptive momentum control)',
            'ilp': 'ON (institutional layer protection)',
            'civ_rate_limiter': 'ON (window=50, max_rate=0.1/step, cooldown=20)',
            'key_fix': 'CIVRateLimiter downgrades excess CIVILIZATION to INSTITUTIONAL',
        },
    ...
    output_path = os.path.join(output_dir, f'exp_100_results_{timestamp}.json')
