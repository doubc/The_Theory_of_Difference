"""experiments/exp_102_nse_quality.py

Phase 4 Experiment 12: NSE Quality Improvements — P1-A

Purpose: Improve NSE quality based on exp_101 P1 deep analysis findings.

exp_101 P1 analysis identified three bottlenecks:
1. SelfHistoryAccumulator: history_depth mean=0.122 (too weak, max_turning_points=50 too high)
2. InstitutionalNarrativeStabilizer: stability=0.937 but lacks discrimination (static=stable)
3. CSCI: pseudo-coherence (σ=0.001 across seeds), CIV layer stability=0, TopDown not activating

Fixes applied:
1. history_max_turning_points: 50→25 (doubles history_depth sensitivity)
2. InstitutionalNarrativeStabilizer: add non-triviality factor
   - Measures narrative label change rate in window
   - Static convergence (change_rate=0) → penalty (0.4)
   - Ideal change rate (5-20%) → full score (1.0)
   - Fragmentation (>35% change) → penalty (0.2-0.6)
3. CIV layer stability: compute from CIV event interval CV
   - Track CIV event steps, compute coefficient of variation of intervals
   - stability = 1 - CV (low CV → periodic CIV → high stability)
4. TopDownConstraint: stability_threshold 0.5→0.3 (allows activation with weaker stability)
5. Ensure CIV layer always present in level_states (even if hierarchy L2 doesn't exist)

Config: Same as exp_101 (N0=72, steps=1600, 8 seeds)
    CSC: ON (exp_95 config + topdown_stability_threshold=0.3)
    GBC: ON (random direction init, soft nudge=0.2)
    NSE: ON (threshold=0.02, odi=0.4, msi=0.3, civ=0.0, gbc=0.1)
        CHANGED: max_turning_points 50→25
        CHANGED: non-triviality factor in stabilizer
    AMC: ON, ILP: ON
    CIVRateLimiterV2: ON (window=50, max_rate=0.1, cooldown=10, min_guarantee=3)

Hypotheses (same H1-H6 + new):
  H1: NSI max > 0.1
  ...
  H5: CIV mean in [3, 15]
  H6: min CIV >= 3
  H7 (new): CSCI std > 0.001 (CSCI should vary across seeds, not be pseudo-coherent)
  H8 (new): TopDown constraints active in at least 2 seeds
"""
...
EXP95_CSC_CONFIG = {
    ...
    'topdown_stability_threshold': 0.3,  # P1: 0.5→0.3
...
class CIVRateLimiterV2(CIVRateLimiter):
    """CIVRateLimiter V2 — with minimum CIV guarantee (same as exp_101)"""
...
class MomentumNarrativeOperatorV4(NarrativeRecursionOperator):
    """V4: CIVRateLimiterV2 (same as exp_101)"""
...
def run_single_seed(N0=72, steps=1600, seed=142, sample_interval=10,
    ...
    if use_nse:
        ...
        # P1 FIX: max_turning_points 50→25
    ...
    # TopDown constraint metrics
    topdown_active_counts = [len(sr.get('cross_scale_coupling', {}).get('top_down_constraints', []))
                             for sr in step_results if 'cross_scale_coupling' in sr]
...
def evaluate_hypotheses(results):
    ...
    h6 = civ_min >= 3
    # H7: CSCI should have meaningful variation across seeds (not pseudo-coherent)
    # exp_101 had CSCI std ≈ 0.001 (pseudo-coherent). We want std > 0.01
    h7 = float(np.mean(csci_stds)) > 0.005
    # H8: TopDown constraints should be active in at least 2 seeds
...
def main():
    ...
    print(f"exp_102: NSE Quality Improvements — P1-A")
    print(f"  N0={N0}, steps={STEPS}, {len(SEEDS)} seeds")
    print(f"  CSC=ON (topdown_threshold=0.3), GBC=ON (random dir), AMC=ON, ILP=ON")
    print(f"  NSE=ON: max_turning_points=25 (was 50), non-triviality factor")
    print(f"  CIV stability from event intervals, CIV layer always in level_states")
    print(f"  Fixes: history_depth sensitivity, stabilizer discrimination, CIV stability, TopDown activation")
    print()
    ...
    # Comparison with exp_101
    print()
    print("COMPARISON WITH exp_101")
    print("-" * 60)
    print(f"  {'Seed':>6} | {'exp_101 NSI':>12} | {'exp_102 NSI':>12} | "
          f"{'exp_101 depth':>13} | {'exp_102 depth':>13} | "
          f"{'exp_101 stab':>12} | {'exp_102 stab':>12}")
    exp_101_nsi = {42: 0.6140, 142: 0.5977, 242: 0.6359, 342: 0.6353,
                   442: 0.6196, 542: 0.6464, 642: 0.6334, 742: 0.5939}
    exp_101_depth = {42: 0.0938, 142: 0.0436, 242: 0.1623, 342: 0.1674,
                     442: 0.1090, 542: 0.1990, 642: 0.1574, 742: 0.0402}
    exp_101_stab = {42: 0.9389, 142: 0.9366, 242: 0.9388, 342: 0.9389,
                    442: 0.9366, 542: 0.9366, 642: 0.9388, 742: 0.9366}
    for r in all_results:
        s = r['seed']
        print(f"  {s:>6} | {exp_101_nsi.get(s, 0):>12.4f} | {r['nse_nsi_max']:>12.4f} | "
              f"{exp_101_depth.get(s, 0):>13.4f} | {r['nse_history_depth_mean']:>13.4f} | "
              f"{exp_101_stab.get(s, 0):>12.4f} | {r['nse_stability_mean']:>12.4f}")

    # Save results
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    output = {
        'experiment': 'exp_102_nse_quality',
        'timestamp': timestamp,
        'config': {
            'N0': N0, 'steps': STEPS, 'seeds': SEEDS,
            'sample_interval': SAMPLE_INTERVAL,
            'gbc_soft_nudge': GBC_SOFT_NUDGE,
            'csc': 'ON (topdown_stability_threshold=0.3)',
            'gbc': 'ON (random direction init)',
            'nse': 'ON (threshold=0.02, odi=0.4, msi=0.3, civ=0.0, gbc=0.1, max_tp=25)',
            'amc': 'ON', 'ilp': 'ON',
            'civ_rate_limiter_v2': 'ON (cooldown=10, min_guarantee=3)',
            'fixes_from_exp_101_p1': [
                'history_max_turning_points 50→25 (doubles history_depth sensitivity)',
                'InstitutionalNarrativeStabilizer: non-triviality factor (penalize static convergence)',
                'CIV layer stability from event interval CV',
                'TopDownConstraint threshold 0.5→0.3',
                'CIV layer always present in level_states',
            ],
        },
    ...
    output_path = os.path.join(output_dir, f'exp_102_results_{timestamp}.json')
