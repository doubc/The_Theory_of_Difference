"""experiments/exp_106_p1e_fixes.py

Phase 4 Experiment 16: P1-E Fixes — CIVRateLimiter Relaxation for H6
...
P1-D fixes:
  ...
  2. CIVRateLimiterV2P1E: max_rate 0.08->0.10, cooldown 8->10 (further relaxation)
...
Config: Same as exp_104 (N0=72, steps=1600, 8 seeds)
    ...
    AMC: ON, ILP: ON
    CIVRateLimiterV2P1E: ON (window=50, max_rate=0.12, cooldown=12, min_guarantee=3)
...
class CIVRateLimiterV2P1E(CIVRateLimiter):
    """CIVRateLimiter V2 P1-D — further relaxed rate limiting

    P1-C used max_rate=0.08, cooldown=8.
    P1-E further relaxes to max_rate=0.12, cooldown=12 to fix H6 (CIV min=2→3).
    """
...
class MomentumNarrativeOperatorV4P1E(NarrativeRecursionOperator):
    """V4 P1-D: CIVRateLimiterV2P1E with further relaxed limits"""
    ...
    def __init__(self, bias_dimension=128, filter_magnitude_threshold=0.02,
        ...
        self.civ_rate_limiter = CIVRateLimiterV2P1E(
...
def run_single_seed(N0=72, steps=1600, seed=142, sample_interval=10,
    ...
    narrative = MomentumNarrativeOperatorV4P1E(
...
def main():
    ...
    print(f"exp_106: P1-E Fixes — INSTITUTIONAL Activity-Driven TopDown & Seal Timeout")
    ...
    print(f"  CIVRateLimiterV2P1E: max_rate=0.10 (was 0.08), cooldown=10 (was 8)")
    print(f"  Fixes: CIVRateLimiter relaxation (max_rate 0.10->0.12, cooldown 10->12) to fix H6 CIV min")
    print()
    ...
    for seed in SEEDS:
        result = run_single_seed(
            ...
            gbc_soft_nudge=GBC_SOFT_NUDGE,
            use_csc=True, csc_config=EXP106_CSC_CONFIG,
            use_nse=True, use_amc=True, use_ilp=True,
        )
    ...
    output = {
        ...
        'config': {
            ...
            'amc': 'ON', 'ilp': 'ON',
            'civ_rate_limiter_v2_p1d': 'ON (max_rate=0.12, cooldown=12, min_guarantee=3)',
            'fixes_from_exp_105': [
                ...
                'CIVRateLimiter: max_rate 0.10->0.12, cooldown 10->12 (P1-E relaxation for H6)',
            ],
        },
    ...
    output_path = os.path.join(output_dir, f'exp_106_results_{timestamp}.json')
