"""experiments/exp_93_cross_scale_coupling_validation.py

Phase 4 Experiment 3: Cross-Scale Coupling Integration Validation

Purpose: Validate that CrossScaleCoupling (CSC) is properly integrated into the
hierarchical evolver and produces meaningful cross-scale coherence signals.
This experiment addresses the fundamental gap identified in exp_92: CSC was
created but never wired into the evolver's main loop.

Changes vs exp_92:
- CSC: Integrated into HierarchicalEvolver._make_phase2_callback
- CSC: Collects level_states (MINI/INSTITUTIONAL/CIVILIZATION) at each step
- CSC: Computes CSCI (Cross-Scale Coherence Index) at each step
- CSC: Tracks Top-Down constraints and Bottom-Up emergence quality
- CSC: Builds narrative bridges across three levels

Config: Same as exp_90 (N0=72, steps=1600, 8 seeds)
    csc_active=true
    ILP: disabled (focus on CSC, not ILP)
    AMC: disabled (focus on CSC, not AMC)
    GBC soft nudge: 0.2 (unchanged from exp_90)

Hypotheses:
  H1: CSCI > 0.3 mean (cross-scale coherence emerges)
  H2: Top-Down constraints active in >= 50% of steps (when high levels exist)
  H3: Narrative bridge coherence > 0.2 mean (narratives align across levels)
  H4: CIV mean >= 5 (same baseline as exp_90, CSC should help or at least not hurt)

Expected: CSC integration produces measurable cross-scale signals without
degrading the baseline exp_90 results. CSC is a soft coupling mechanism
that should improve CIV min (H4) by providing cross-scale feedback.
"""
...
def main():
    ...
    print("=" * 70)
    print("exp_93: Cross-Scale Coupling Integration Validation")
    print("=" * 70)
    print(f"Seeds: {seeds}")
    print(f"N0={N0}, steps={steps}")
    print(f"CSC config: {EXP93_CSC_CONFIG}")
    ...
    print(f"\nGBC coherence mean: {np.mean(gbc_coh):.4f}")
    ...
    h2_pass = np.mean(csc_csci) >= 0.3
    h3_pass = np.mean(csc_narr) >= 0.2
    h4_pass = min(civ_values) >= 3

    print(f"H1 (mean CIV >= 5):     {'PASS' if h1_pass else 'FAIL'} ({np.mean(civ_values):.2f})")
    print(f"H2 (CSC CSCI >= 0.3):   {'PASS' if h2_pass else 'FAIL'} ({np.mean(csc_csci):.4f})")
    print(f"H3 (Narrative coh >= 0.2): {'PASS' if h3_pass else 'FAIL'} ({np.mean(csc_narr):.4f})")
    print(f"H4 (min CIV >= 3):      {'PASS' if h4_pass else 'FAIL'} (min={min(civ_values)})")

    # vs exp_90 comparison
    print("\n--- vs exp_90 ---")
    print(f"CIV mean:    {np.mean(civ_values):.2f} (exp_90: 5.25)")
    print(f"CIV min:     {min(civ_values)} (exp_90: 2)")
    print(f"GBC coh:     {np.mean(gbc_coh):.4f} (exp_90: 0.572)")
    print(f"GBC pass:    {np.mean(gbc_pr):.4f} (exp_90: 0.375)")

    # Save results
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    output_path = os.path.join(
        PROJECT_ROOT, 'experiments', f'exp_93_results_{timestamp}.json'
    )
    output = {
        'experiment': 'exp_93_cross_scale_coupling_validation',
        'timestamp': timestamp,
        'seeds': seeds,
        'n_seeds': len(seeds),
        'config': {
