"""
experiments/exp_138_phase8_p1_l1_cycle_detection.py

Phase 8 P1: L1 Cycle Detection via Encapsulation-Aware Callback

Purpose:
  Phase 8 P1 (exp_138) established the zero baseline: with max_layers=1 and the
  standard LCylDetTrackingCallback, L1 cycles were NEVER detected (0/8 seeds)
  because the evolver never fires the tracking callback for layer_id=1.

  Phase 8 P1 fixes this with encapsulation-aware callback: when l1_formed=True
  and layer_id=0, it extracts L1 metrics directly from the evolver hierarchy
  (hierarchy.get_layer(1)), computing L1 NSI/CIV/active_bits from the encapsulated
  LayerState, feeding them to L1CycleDetector for proper cycle detection.

Hypotheses:
  H86 (L1 cycles): >=6/8 seeds with >=1 L1 cycle events (any type) in 2000 steps
  H86a (L1 cycle types): >=4/8 seeds with >=2 cycle types active
  H86b (L1 cycle spacing): Mean spacing <= 500 steps (if cycles exist)
  H89 (No degradation): >=6/8 seeds pass all H1-H8 baseline hypotheses

Core Question:
  Does the encapsulation-aware callback properly extract L1 dynamics from the
  hierarchy encapsulated LayerState?

Config:
  Same as P0 (N0=72, CSC+NSE+NRC+Booster, tension=1.0, max_layers=1)
  - L1Aware tracking callback (extracts from hierarchy.get_layer(1))
  - L1CycleDetector (monitor only, no feedback)
  - PerLayerMetricsCollector (feeds L0 data as usual)
"""
...
from engine.l1_cycle_detector import (
    L1CycleDetector, L1AwareLCylDetCallback,
...
def extract_l1_metrics(tracking_collector, l1_callback):
    """Extract L1 metrics from L1AwareLCylDetCallback (has get_l1_metrics)."""
    if l1_callback is not None and hasattr(l1_callback, 'get_l1_metrics'):
        return l1_callback.get_l1_metrics()
    return {
        'l1_detector': {'total_cycles': 0, 'by_type': {}},
        'l1_cycle_times': {},
        'n_cycle_types_active': 0,
        'l1_last_nsi': 0.0,
        'l1_last_civ': 0,
        'l1_extracted': False,
    }
...
class LCylDetTrackingCallback:
    """
    Encapsulation-aware tracking callback for L1 cycle detection (Phase 8 P1).

    Extracts L1 metrics from evolver hierarchy when l1_formed=True and layer_id=0,
    using a hierarchy_getter callable that resolves the hierarchy at call time.
    This works around the issue that with max_layers=1, the evolver never fires
    the tracking callback for layer_id=1.
    """
    def __init__(self, collector, l1_detector, hierarchy_getter=None):
        self.collector = collector
        self.l1_detector = l1_detector
        self.hierarchy_getter = hierarchy_getter
    ...
    def step(self, step, layer_id, n_active, n_total, n_frozen,
        ...
        import sys as _sys
        if l1_formed and layer_id == 0 and self.l1_detector and self.hierarchy_getter:
            print(f"[DBG] l1_formed=True at step={step}, extracting...", file=_sys.stderr, flush=True)
            try:
                ...
                if hierarchy and hierarchy.n_layers > 1:
                    ...
                    l1_nsi = l1_active / max(l1_total, 1) if l1_total > 0 else 0.0
                    l1_civ = l1_active
                ...
                import sys
                print(f"[DBG cb] l1_extract failed: {e}", file=sys.stderr, flush=True)

    def get_l1_metrics(self):
        ...
        type_counts = {
            'reconfiguration': cycle_times.get('reconfiguration', []),
            'reshuffle': cycle_times.get('reshuffle', []),
            'identity_shift': cycle_times.get('identity_shift', []),
        }
        n_types = sum(1 for v in type_counts.values() if len(v) > 0)
        return {
            'l1_detector': summary,
...
def run_single_seed(N0, steps, seed, sample_interval, gbc_soft_nudge,
                    ...
                    tracking_callback_obj=None, evolver_ref=None):
    """
    Run a single seed with L1CycleDetector monitoring.

    Parameters
    ----------
    evolver_ref : optional container with .evolver attribute
        If provided, evolver_ref.evolver is set to the created evolver
        for use by the tracking callback hierarchy_getter.
    """
    ...
    if evolver_ref is not None:
        evolver_ref.evolver = evolver
    ...
    l1_metrics = extract_l1_metrics(tracker, tracking_callback_obj)
...
def evaluate_hypotheses(results, label="phase8_p1"):
...
def main():
    """Run Phase 8 P1: L1 Cycle Detection experiment."""
    print("=" * 70)
    print("exp_138: PHASE 8 P1 — L1 Cycle Detection (LCylDet Monitor Only)")
    print("=" * 70)
    ...
    for seed in SEEDS:
        ...
        class EvolverRef:
            """Simple container so the callback lambda resolves hierarchy at call time."""
            def __init__(self):
                self.evolver = None

        evolver_ref = EvolverRef()
        tracking_cb = LCylDetTrackingCallback(
            collector=tracking_collector,
            l1_detector=l1_detector,
            hierarchy_getter=lambda: evolver_ref.evolver.hierarchy if evolver_ref.evolver else None,
        )
    ...
    # Evaluate hypotheses
    hypotheses = evaluate_hypotheses(results, 'phase8_p1_l1_baseline')

    # --- Summary ---
    print("\n" + "=" * 70)
    print("PHASE 8 P1 RESULTS — L1 Cycle Detection Summary")
    print("=" * 70)
    ...
    print(f"  H86b (cycle frequency): {'PASS' if h.get('H86b_cycle_spacing', {}).get('pass') else 'FAIL'} — {h.get('H86b_cycle_spacing', {}).get('value', '?')}")
    print(f"  H89 (no degradation): {'PASS' if h.get('H89_no_degradation', {}).get('pass') else 'FAIL'} — {h.get('H89_no_degradation', {}).get('value', '?')}")
    print(f"  L1 extracted seeds: {h.get('l1_extracted_seeds', 0)}/8")
    ...
    print(f"  Phase 8 P1: {p8['n_pass']}/4 PASS {'[PASS]' if p8['all_pass'] else '[X] ' + ', '.join(p8['failed'])}")
    ...
    results_file = os.path.join(
        ...
        'exp_138_phase8_p1_l1_baseline_%s.json' % timestamp)
    save_data = {
        'experiment': 'exp_138_phase8_p1_l1_cycle_baseline',
    ...
    print(f"\n{'=' * 70}")
    print("COMPARISON: Phase 7 (no L1 monitoring) vs Phase 8 P1 (LCylDet monitor)")
    print(f"{'=' * 70}")
    ...
    print(f"  Phase 8 P1 (exp_138 N0=72): L1_cycles={l1_total}, NRC_R2={nrc_r2}, NRC_cycles={nrc_cyc}")
