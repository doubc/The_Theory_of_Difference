"""
exp_163 — Phase 15 Path A1: Sigmoid Binding (Nonlinear Feedback)

科学问题：
  Phase 14 证明死秩序是单层世界的拓扑不变性 — 指数衰减的约束信号被密封结构「吸收」。
  一个可能的解释是：指数衰减的连续特性使得反馈信号无法在密封边界处产生足够的非线性跳变。

核心假设：
  H15-A1 (Sigmoid 假设)：Sigmoid 型约束信号（相变式跳变）能在密封边界产生
    足够的非线性跳变，触发二次密封。
  H15-A2 (陡峭度假设)：陡峭 Sigmoid (steepness=0.1) 最接近 step 函数，
    在 threshold ≈ decay_length 处效果最强。

反馈机制：
  bias[i] = fs / (1 + exp((dist - threshold) / steepness))
  其中 dist = 到最近密封比特的距离（线性距离）

实验设计：
  - 16 种 (threshold × steepness) 组合 × 2 种 fs = 32 条件
  - 每个条件 2 次运行 = 64 runs
  - N=48, 总步数 8000 (密封前后各 4000)

测量指标：
  - 二次密封率 (secondary_sealing_rate)
...
用法：
  ...
  python experiments/exp_163_phase15_a1_sigmoid_binding.py
"""
...
# ============================================================
N = 48
TOTAL_STEPS = 1200  # 密封很快发生(~100步)，1200步足以观察后密封动力学
SAMPLE_INTERVAL = 50
N_RUNS = 1          # 每条件运行次数
...
# Sigmoid 反馈条件
# ============================================================
# threshold: Sigmoid 中间点（相变边界）
# steepness: Sigmoid 陡峭度（越小越陡 → 越接近 step 函数）
THRESHOLDS = [2, 4, 8, 16]
STEEPNESSES = [0.1, 0.5, 1.0, 2.0]
    ...
    for st in STEEPNESSES:
        for fs in FEEDBACK_STRENGTHS:
            FEEDBACK_CONDITIONS.append({
                'label': f'sig_th{th}_st{st}_fs{fs}',
                'threshold': th,
                'steepness': st,
...
# Sigmoid 反馈回调
# ============================================================
def make_sigmoid_callback(threshold: int, steepness: float, feedback_strength: float):
    """创建 Sigmoid 型主动反馈 step_callback

    bias[i] = fs / (1 + exp((dist - threshold) / steepness))
    当 dist < threshold: bias ≈ fs（强约束）
    当 dist > threshold: bias ≈ 0（弱约束）
    当 dist ≈ threshold: bias 在 fs/2 附近（相变区）

    物理意义：
      密封结构周围形成一个「约束场」，在 threshold 距离处突然增强。
      这种非线性跳变可能打破指数衰减的平滑特性，触发相变。
    """
    def _callback(step: int, state: torch.Tensor,
        ...
        for i in range(N_state):
            ...
            # Sigmoid: fs / (1 + exp((dist - threshold) / steepness))
            if steepness > 0:
                exponent = (min_dist - threshold) / steepness
                # 防止 exp 溢出
                exponent = max(-50.0, min(50.0, exponent))
                bias[i] = feedback_strength / (1.0 + math.exp(exponent))
            else:
                # steepness=0 → step 函数
                bias[i] = feedback_strength if min_dist <= threshold else 0.0
...
def run_single(condition: dict, run_id: int) -> dict:
    ...
    st = condition['steepness']
    fs = condition['feedback_strength']

    seed = run_id * 31337 + 163 + hash(label) % 100000
    ...
    callback = make_sigmoid_callback(th, st, fs)
    ...
    # ---- 二次密封检测 (sealed_ratio > 0.40 的视为过密封) ----
...
def print_results(analysis: dict):
    print(f"\n{'=' * 110}")
    print("Phase 15 Path A1: Sigmoid Binding — 结果分析 (exp_163)")
    print("=" * 110)
    ...
    # 排名表
    header = (f"{'Condition':32s} {'Seal':>6s} {'FlipRate':>8s} {'Bound%':>7s} "
              f"{'Dist':>5s} {'HWvar':>7s} {'OverS':>6s} {'DirBal':>7s} {'ξ':>5s}")
    print(f"\n{header}")
    print("-" * 110)
    ...
    for item in ranked:
        ...
        print(f"  {label:32s} {se:>6s} {fr:>8s} {br:>7s} {dd:>5s} {hv:>7s} {os_:>6s} {db:>7s} {xi:>5s}")
    ...
    for th in THRESHOLDS:
        th_conds = [cs for lbl, cs in by_cond.items()
                    if lbl.startswith(f'sig_th{th}_')]
        if not th_conds:
            continue
    ...
    print(f"\n  --- By Steepness ---")
    for st in STEEPNESSES:
        st_conds = [cs for lbl, cs in by_cond.items()
                    if f'_st{st}_' in lbl]
        if not st_conds:
            continue
        avg_flip = np.mean([c['post_flip_rate_mean'] for c in st_conds])
        avg_bound = np.mean([c['boundary_flip_ratio_mean'] for c in st_conds])
        avg_over = np.mean([c['over_sealed_rate'] for c in st_conds])
        print(f"    steepness={st:.1f}: flip_rate={avg_flip:.4f}, "
              f"boundary%={avg_bound*100:.1f}%, "
    ...
    # 最佳条件对比
    best = ranked[0] if ranked else {'label': 'None', 'post_flip_rate': 0, 'boundary_ratio': 0}
    best_label = best['label']
    best_flip = best['post_flip_rate']
    best_bound = best['boundary_ratio']
    ...
    if max_over > 0.2:
        print(f"  [STRONG] H15-A1 (Sigmoid): OVER-SEALED DETECTED — "
              f"max_over_sealed_rate={max_over*100:.0f}%")
    else:
        print(f"  [NO] H15-A1 (Sigmoid): No secondary sealing detected — "
              f"max_over_sealed_rate={max_over*100:.0f}%")
    ...
    if best_bound_z > 2.0:
        print(f"  [MEDIUM] H15-A2 (Steepness): Boundary enrichment detected — "
              f"z={best_bound_z:.2f}")
    else:
        print(f"  [NO] H15-A2 (Steepness): No significant boundary enrichment — "
              f"z={best_bound_z:.2f}")
    ...
    # 综合结论
    all_no = (max_over <= 0.2 and best_bound_z <= 2.0 and max_dir_dev < 0.1)
    if all_no:
        print(f"\n  === FINAL: Path A1 REJECTED — Sigmoid binding fails to break dead order ===")
    else:
        print(f"\n  === FINAL: Path A1 SHOWS SIGNAL — further investigation warranted ===")
...
def run_experiment():
    print("=" * 110)
    print("Phase 15 Path A1: Sigmoid Binding (exp_163)")
    print("=" * 110)
    ...
    print(f"  Total runs: {len(FEEDBACK_CONDITIONS) * N_RUNS}")
    print(f"  Sigmoid: bias = fs / (1 + exp((dist - threshold) / steepness))")
    print(f"  threshold ∈ {THRESHOLDS}  steepness ∈ {STEEPNESSES}  fs ∈ {FEEDBACK_STRENGTHS}")
    print("=" * 110)
    ...
    sigmoid_conds = [c for c in FEEDBACK_CONDITIONS if c['label'] != 'baseline']

    for cond in baseline_cond + sigmoid_conds:
        label = cond['label']
        print(f"\n--- {label} ---")
    ...
    result_file = os.path.join(RESULTS_DIR,
                               f"exp163_phase15_a1_sigmoid_{timestamp}.json")
    ...
    save_data = {
        'params': {
            ...
            'timestamp': timestamp,
            'experiment': 'exp_163_phase15_a1_sigmoid_binding',
        },
        'analysis': {
