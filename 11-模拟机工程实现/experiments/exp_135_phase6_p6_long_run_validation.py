"""
experiments/exp_135_phase6_p6_long_run_validation.py

Phase 6 P6: Long-Run Validation for H63 (Spiral Convergence) + H64 (Completeness)

Purpose:
  exp_134 proved tension-based R2 trigger works (100% activation at tension<=1.5),
  but H63 (convergence) and H64 (completeness >=3 cycles/1k) were not validated
  due to insufficient run length (3000 steps).
  
  Key insight from exp_129: Cycles cluster in first 200 steps, then go silent.
  But with tension-based R2 (exp_133/134), R2 fires at cumulative tension threshold,
  NOT at cycle_count > 5. So longer runs may show:
  1. More total cycles (H64: >=3 cycles per 1000 steps)
  2. Spiral convergence (H63: weight diffs decrease over cycles)
  
  This experiment runs 8 seeds x 8000 steps at the recommended tension=1.0
  with NarrativeLevelBooster to give the best chance of H63/H64 validation.

Hypotheses:
  H1-H8: Core emergence preserved (baseline check)
  H63 (Convergence): >=6/8 seeds show negative slope of weight diffs
  H64 (Completeness): >=6/8 seeds have >=3 complete E->M->S->R cycles per 1000 steps
  H62 (R2 activation): >=4/8 seeds trigger R2 (redundant with exp_134, but re-verify)
  H79 (Long-run stability): H1-H8 pass at 8000 steps (no degradation over time)
  H80 (Cycle accumulation): total cycles at 8000 steps >= 2x total at 3000 steps
"""
...
def extract_nrc_metrics(evolver, step_results):
    ...
    # Cycle step distribution (for H80: cycle accumulation)
    cycle_steps = [c.step for c in cycles]
    metrics['cycle_step_distribution'] = cycle_steps
...
def run_single_seed(N0, steps, seed, sample_interval, gbc_soft_nudge,
    ...
    """Run a single seed with CSC+NSE+NRC (tension-based R2 trigger) + Booster."""
...
def evaluate_hypotheses(results, label="unknown"):
    """Evaluate H1-H8 + H62-H64 + H79-H80."""
    if not results:
    ...
    # H79: Long-run stability (H1-H8 all pass at 8000 steps)
    h79 = h1 and h2 and h3 and h4 and h5 and h6 and h7 and h8

    # H80: Cycle accumulation (total cycles >= 2x exp_134 baseline)
    # exp_134 tension_1.0 had mean ~12 R2 events across 8 seeds in 3000 steps
    # With 8000 steps, expect >= 24 total R2 (2x)
    h80 = h62_total_r2 >= 24
    ...
    return {
        ...
        'H63_convergence': {
            'value': '%d/8 converges' % sum(1 for c in converges_list if c),
            'threshold': '>=6 seeds', 'pass': h63},
        'H64_completeness': {
            'value': 'cycles/1k=%s' % ', '.join('%.1f' % r for r in cycles_per_1k),
            'threshold': '>=6 seeds >=3.0', 'pass': h64},
        'H79_long_run_stability': {
            'value': '%d/8 H1-H8 pass' % sum([h1, h2, h3, h4, h5, h6, h7, h8]),
            'threshold': '8/8', 'pass': h79},
        'H80_cycle_accumulation': {
            'value': 'total_R2=%d' % h62_total_r2,
            'threshold': '>=24 (2x exp_134)', 'pass': h80},
        'nrc_aggregate': {
        ...
        'p6_summary': {
            'n_pass': sum([h62, h63, h64, h79, h80]),
            'all_pass': h62 and h63 and h64 and h79 and h80,
            'failed': [n for n, v in [('H62', h62), ('H63', h63), ('H64', h64), ('H79', h79), ('H80', h80)] if not v],
        },
    }


# --- Experiment matrix ---
# Single config: tension=1.0 (recommended default from exp_134)
# 8 seeds x 8000 steps = 8 runs
# Compare against exp_134's tension_1.0 results (3000 steps)
...
GBC_SOFT_NUDGE = 0.2
N0 = 48
R2_TENSION_THRESHOLD = 1.0  # recommended default from exp_134


def main():
    print("=" * 70)
    print("exp_135: Phase 6 P6 — Long-Run Validation (8000 steps)")
    print("  Architecture: CSC+NSE+NRC+NarrativeLevelBooster")
    print("  N0=%d  Seeds: %d  Steps: %d" % (N0, len(SEEDS), N_STEPS))
    print("  R2 tension_threshold=%.1f (recommended default)" % R2_TENSION_THRESHOLD)
    print("  Hypotheses: H1-H8 + H60-H64 + H79-H80")
    ...
    for seed in SEEDS:
        try:
            ...
            cycles_per_1k = nrc_summary.get('h64_cycles_per_1000', 0.0)
            conv_slope = nrc_summary.get('h63_convergence_slope', 0.0)
            print(f"    seed={seed}: NSI_max={result['nse_nsi_max']:.4f}, "
                  f"peak_NSI={peak_nsi:.4f}, "
                  f"R2={n_r2}, cycles={n_cyc}, "
                  f"tension={tension:.2f}, "
                  f"cycles/1k={cycles_per_1k:.1f}, "
                  f"conv_slope={conv_slope:.6f}, "
    ...
    # Evaluate hypotheses
    hypotheses = evaluate_hypotheses(results, 'tension_1.0_8000steps')

    # --- Summary ---
    print("\n" + "=" * 70)
    print("PHASE 6 P6 RESULTS — Long-Run Validation Summary")
    print("=" * 70)
    ...
    print(f"  H62 (R2 activation): {'PASS' if h.get('H62_r2_activation', {}).get('pass') else 'FAIL'} — {h.get('H62_r2_activation', {}).get('value', '?')}")
    print(f"  H63 (convergence): {'PASS' if h.get('H63_convergence', {}).get('pass') else 'FAIL'} — {h.get('H63_convergence', {}).get('value', '?')}")
    print(f"  H64 (completeness): {'PASS' if h.get('H64_completeness', {}).get('pass') else 'FAIL'} — {h.get('H64_completeness', {}).get('value', '?')}")
    print(f"  H79 (long-run stability): {'PASS' if h.get('H79_long_run_stability', {}).get('pass') else 'FAIL'} — {h.get('H79_long_run_stability', {}).get('value', '?')}")
    print(f"  H80 (cycle accumulation): {'PASS' if h.get('H80_cycle_accumulation', {}).get('pass') else 'FAIL'} — {h.get('H80_cycle_accumulation', {}).get('value', '?')}")
    ...
    for r in results:
        ...
        print(f"  seed={r.get('seed', '?'):>3}: "
              ...
              f"cycles/1k={nrc_m.get('h64_cycles_per_1000', 0):.1f} "
              f"conv={nrc_m.get('h63_converges', False)} "
              f"slope={nrc_m.get('h63_convergence_slope', 0):.6f} "
    ...
    results_file = os.path.join(
        ...
        'exp_135_phase6_p6_long_run_%s.json' % timestamp)
    save_data = {
        'experiment': 'exp_135_phase6_p6_long_run_validation',
    ...
    # --- Comparison with exp_134 ---
    print(f"\n{'=' * 70}")
    print("COMPARISON: exp_134 (3000 steps) vs exp_135 (8000 steps)")
    print(f"{'=' * 70}")
    print(f"  exp_134 tension_1.0: 8/8 R2, total=12 events in 3000 steps")
    print(f"  exp_135 tension_1.0: {sum(1 for r in results if r.get('nrc_metrics', {}).get('n_r2_events', 0) >= 1)}/8 R2, "
          f"total={sum(r.get('nrc_metrics', {}).get('n_r2_events', 0) for r in results)} events in 8000 steps")
    exp134_cycles_per_1k = 12.0 / (3000 / 1000.0) / 8  # ~0.5 per seed
    exp135_cycles_per_1k = nrc_agg.get('nrc_cycles_total', 0) / (8000 / 1000.0) / 8
    print(f"  Cycles/1k per seed: exp_134=%.2f, exp_135=%.2f" % (exp134_cycles_per_1k, exp135_cycles_per_1k))
