"""experiments/exp_103_p1b_fixes.py

Phase 4 Experiment 13: P1-B Fixes — CIV Stability & TopDown Activation

Purpose: Fix H5 (CIV explosion) and H8 (TopDown not activating) from exp_102.

exp_102 results: 6/8 pass
  H5 FAIL: CIV mean=32.125 (seed 742 civ=194) — non-triviality too aggressive
  H8 FAIL: TopDown active seeds=0 — CIV layer stability insufficient

P1-B fixes:
  1. Moderate non-triviality factor (floor 0.4->0.6, ceiling unchanged)
     - Prevents stability from collapsing, keeps CIVRateLimiter effective
  2. Tighten CIVRateLimiterV2: max_civ_rate 0.1->0.05, cooldown 10->5
     - More aggressive rate limiting to handle increased CIV generation
  3. CIV stability: combine interval CV with event density
     - interval_stability = 1 - CV of intervals (existing)
     - density_stability = min(1.0, n_events / 20) (new)
     - combined = 0.6 * interval_stability + 0.4 * density_stability
     - More robust than interval-only (which fails with few events)
  4. TopDown threshold: 0.3 -> 0.15
     - Further lowered to allow activation with weaker stability signals
  5. INSTITUTIONAL stability: use protector's stability_score as fallback
     - When hierarchy INSTITUTIONAL layer lacks stability, use ILP's internal score

Config: Same as exp_102 (N0=72, steps=1600, 8 seeds)
    CSC: ON (topdown_stability_threshold=0.15)
    GBC: ON (random direction init, soft nudge=0.2)
    NSE: ON (threshold=0.02, odi=0.4, msi=0.3, civ=0.0, gbc=0.1, max_tp=25)
        CHANGED: moderated non-triviality factor
    AMC: ON, ILP: ON
    CIVRateLimiterV2: ON (window=50, max_rate=0.05, cooldown=5, min_guarantee=3)
...
EXP95_CSC_CONFIG = {
    ...
    'topdown_stability_threshold': 0.15,  # P1-B: 0.3 -> 0.15
...
class CIVRateLimiterV2P1B(CIVRateLimiter):
    """CIVRateLimiter V2 P1-B — tighter rate limiting"""

    def __init__(self, window_size=50, max_civ_rate=0.05, cooldown_steps=5,
                 min_civ_guarantee=3):
...
class MomentumNarrativeOperatorV4P1B(NarrativeRecursionOperator):
    """V4 P1-B: CIVRateLimiterV2P1B with tighter limits"""
    ...
    def __init__(self, bias_dimension=128, filter_magnitude_threshold=0.02,
        ...
        self.civ_rate_limiter = CIVRateLimiterV2P1B(
            window_size=50, max_civ_rate=0.05, cooldown_steps=5,
            min_civ_guarantee=3
        )
...
def run_single_seed(N0=72, steps=1600, seed=142, sample_interval=10,
    ...
    narrative = MomentumNarrativeOperatorV4P1B(
...
def main():
    ...
    print(f"exp_103: P1-B Fixes — CIV Stability & TopDown Activation")
    print(f"  N0={N0}, steps={STEPS}, {len(SEEDS)} seeds")
    print(f"  CSC=ON (topdown_threshold=0.15), GBC=ON (random dir), AMC=ON, ILP=ON")
    print(f"  NSE=ON: max_turning_points=25, moderated non-triviality")
    print(f"  CIVRateLimiterV2P1B: max_rate=0.05 (was 0.1), cooldown=5 (was 10)")
    print(f"  Fixes: non-triviality floor 0.4->0.6, tighter limiter, CIV stability combo, TopDown thresh 0.15")
    print()
    ...
    # Comparison with exp_102
    print()
    print("COMPARISON WITH exp_102")
    print("-" * 60)
    print(f"  {'Seed':>6} | {'e102 NSI':>10} | {'e103 NSI':>10} | "
          f"{'e102 stab':>10} | {'e103 stab':>10} | "
          f"{'e102 civ':>9} | {'e103 civ':>9} | "
          f"{'e102 td':>8} | {'e103 td':>8}")
    exp_102_nsi = {42: 0.7379, 142: 0.6126, 242: 0.6796, 342: 0.5680,
                   442: 0.7653, 542: 0.6453, 642: 0.5920, 742: 0.7036}
    exp_102_stab = {42: 0.4827, 142: 0.4796, 242: 0.4831, 342: 0.4823,
                    442: 0.4796, 542: 0.4796, 642: 0.4823, 742: 0.4796}
    exp_102_civ = {42: 6, 142: 15, 242: 7, 342: 5, 442: 5, 542: 7, 642: 18, 742: 194}
    exp_102_td = {42: 0, 142: 0, 242: 0, 342: 0, 442: 0, 542: 0, 642: 0, 742: 0}
    for r in all_results:
        s = r['seed']
        print(f"  {s:>6} | {exp_102_nsi.get(s, 0):>10.4f} | {r['nse_nsi_max']:>10.4f} | "
              f"{exp_102_stab.get(s, 0):>10.4f} | {r['nse_stability_mean']:>10.4f} | "
              f"{exp_102_civ.get(s, 0):>9} | {r['civ_count']:>9} | "
              f"{exp_102_td.get(s, 0):>8} | {r['topdown_max_active']:>8}")

    # Save results
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    output = {
        'experiment': 'exp_103_p1b_fixes',
        'timestamp': timestamp,
        'config': {
            ...
            'amc': 'ON', 'ilp': 'ON',
            'civ_rate_limiter_v2_p1b': 'ON (max_rate=0.05, cooldown=5, min_guarantee=3)',
            'fixes_from_exp_102': [
                'non-triviality factor: floor 0.4->0.6 (less aggressive)',
                'CIVRateLimiterV2: max_rate 0.1->0.05, cooldown 10->5',
                'CIV stability: combined interval CV + event density',
                'TopDown threshold: 0.3->0.15',
                'INSTITUTIONAL stability: use ILP score as fallback',
            ],
        },
    ...
    output_path = os.path.join(output_dir, f'exp_103_results_{timestamp}.json')
