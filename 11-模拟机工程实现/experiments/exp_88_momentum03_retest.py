"""experiments/exp_88_momentum03_retest.py

Phase 3 Experiment 16: Momentum bonus=0.3 retest with coherence_threshold=0.5

exp_87 showed momentum_bonus=0.5 gives mean CIV=4.25 (worse than exp_85's 9.0).
Hypothesis: bonus=0.5 over-reinforces hotspots, trapping in local attractors.
Bonus=0.3 (from exp_85) may be the sweet spot.

This experiment tests bonus=0.3 with coherence_threshold=0.5
on the 4 marginal seeds from exp_87 that had CIV < 5.

Design:
  - Seeds: [242, 442, 642, 742] (the marginal/failing seeds from exp_87)
  - N0=72, steps=1600, sample_interval=10
  - momentum_bonus=0.3 (revert from 0.5)
  - momentum_decay=0.95
  - coherence_threshold=0.5

Hypotheses:
  H1: All 4 seeds achieve CIV >= 5 (bonus=0.3 more robust)
  H2: Mean CIV >= 6 across 4 seeds
  H3: GBC coherence mean >= 0.55 (maintained from exp_87)
"""
...
def main():
    print("=" * 70)
    print("exp_88: Momentum bonus=0.3 retest (marginal seeds, threshold=0.5)")
    print("=" * 70)
    ...
    # Per-seed comparison with exp_87
    exp_87_civ = {242: 3, 442: 2, 642: 2, 742: 4}
    print(f"\n  {'Seed':>6} | {'CIV':>5} | {'exp_87':>7} | {'Delta':>6} | Status", flush=True)
    print(f"  {'-'*6}-+-{'-'*5}-+-{'-'*7}-+-{'-'*6}-+-{'-'*6}", flush=True)
    for r in all_results:
        status = "PASS" if r['civilization_steps'] >= 5 else "FAIL"
        delta = r['civilization_steps'] - exp_87_civ.get(r['seed'], 0)
        delta_str = f"+{delta}" if delta >= 0 else str(delta)
        print(f"  {r['seed']:>6} | {r['civilization_steps']:>5} | {exp_87_civ.get(r['seed'], '?'):>7} | {delta_str:>6} | {status}", flush=True)

    h1 = all(r['civilization_steps'] >= 5 for r in all_results)
    h2 = civ_mean >= 6.0
    h3 = float(np.mean(gbc_coh_values)) >= 0.55

    print(f"\n  H1 (all CIV >= 5): {'PASS' if h1 else 'FAIL'}", flush=True)
    print(f"  H2 (mean CIV >= 6): {'PASS' if h2 else 'FAIL'} ({civ_mean:.1f})", flush=True)
    print(f"  H3 (GBC coh >= 0.55): {'PASS' if h3 else 'FAIL'} ({float(np.mean(gbc_coh_values)):.4f})", flush=True)

    print(f"\n  vs exp_87 (bonus=0.5):", flush=True)
    for r in all_results:
        exp87_civ = exp_87_civ.get(r['seed'], '?')
        print(f"    seed={r['seed']}: exp_87={exp87_civ} -> exp_88={r['civilization_steps']}", flush=True)

    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                f'exp_88_results_{ts}.json')
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump({
            'experiment': 'exp_88_momentum03_retest',
            'timestamp': ts,
            'seeds': seeds,
            'n_seeds': len(seeds),
            'config': {
