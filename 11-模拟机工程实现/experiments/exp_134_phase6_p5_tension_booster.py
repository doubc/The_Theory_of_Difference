"""
experiments/exp_134_phase6_p5_tension_booster.py

Phase 6 P5: Tension-Based R2 Trigger + NarrativeLevelBooster

Purpose:
  exp_133 proved tension-based R2 trigger works WITHOUT Booster (32+ R2 events).
  exp_129 proved R2 is dormant WITH Booster but using old NSI-based trigger.
  This experiment combines both: tension-based R2 + NarrativeLevelBooster.

  Key question: Does the Booster interfere with tension-based R2 activation?
  The Booster forces min_civ=3, which generates CIV events. These events
  contribute to cumulative_tension, so we expect R2 to trigger even more
  reliably than in exp_133.

Hypotheses:
  H1-H8: Core emergence preserved with booster (baseline check)
  H62 (R2 activation): >=4/8 seeds trigger R2 with tension threshold=1.5
  H63 (Convergence): P_{t+1} - P_t decreases over cycles (negative slope)
  H64 (Completeness): >=3 complete E->M->S->R cycles per 1000 steps
  H77 (Booster synergy): R2 events with Booster >= R2 events without Booster
  H78 (Core preserved): H1-H8 all pass with booster active
"""
...
def run_single_seed(N0, steps, seed, sample_interval, gbc_soft_nudge,
    ...
    # *** NarrativeLevelBooster (Phase 6 P5: tension + booster) ***
...
def evaluate_hypotheses(results, label="unknown"):
    ...
    # H77: Booster synergy (R2 activation >= 75%)
    h77 = sum(1 for cnt in all_r2_counts if cnt >= 1) >= 6

    # H78: Core preserved with booster
    h78 = h1 and h2 and h3 and h4 and h5 and h6 and h7 and h8
    ...
    return {
        ...
        'H77_booster_synergy': {
            'value': '%d/8 seeds with R2' % sum(1 for v in all_r2_counts if v >= 1),
            'threshold': '>=6 seeds', 'pass': h77},
        'H78_core_preserved_with_booster': {
            'value': '%d/8 H1-H8 pass' % sum([h1, h2, h3, h4, h5, h6, h7, h8]),
            'threshold': '8/8', 'pass': h78},
        'nrc_aggregate': {
        ...
        'p6_summary': {
            'n_pass': sum([h73, h74, h75, h76, h77, h78]),
            'all_pass': h73 and h74 and h75 and h76 and h77 and h78,
            'failed': [n for n, v in [('H73', h73), ('H74', h74), ('H75', h75), ('H76', h76), ('H77', h77), ('H78', h78)] if not v],
        },
        'phase6_full_summary': {
            'n_pass': sum([h60, h61, h62, h63, h64, h73, h74, h75, h76, h77, h78]),
            'all_pass': all([h60, h61, h62, h63, h64, h73, h74, h75, h76, h77, h78]),
            'h62_pass': h62, 'h63_pass': h63, 'h64_pass': h64,
        },
    }
...
def main():
    print("=" * 70)
    print("exp_134: Phase 6 P5 — Tension-Based R2 Trigger + Booster")
    ...
    print("  Hypotheses: H1-H8 + H60-H64 + H73-H78")
    ...
    # --- Summary ---
    print("\n" + "=" * 70)
    print("PHASE 6 P5 RESULTS — Tension-Based R2 Trigger Summary")
    print("=" * 70)
    ...
    results_file = os.path.join(
        ...
        'exp_134_phase6_p5_tension_booster_%s.json' % timestamp)
    save_data = {
        'experiment': 'exp_134_phase6_p5_tension_booster',
