"""experiments/exp_89_gbc_soft_nudge.py

Phase 3 Experiment 17: GBC soft nudge for negative-coherence mechanisms

exp_88 showed selection mechanism has negative coherence (anti-correlated
with global bias). This experiment tests a "soft nudge" approach:
after GBC evaluation, negatively-coherent mechanisms get their bias
vectors gently rotated toward the global direction.

Design:
  - Seeds: [242, 442, 642, 742] (marginal seeds from exp_87/88)
  - N0=72, steps=1600, sample_interval=10
  - momentum_bonus=0.3 (best so far)
  - coherence_threshold=0.5
  - gbc_soft_nudge=0.2 (gentle rotation toward global direction)

Hypotheses:
  H1: Mean CIV >= 5.0 (maintained from exp_88)
  H2: GBC coherence mean >= 0.55 (improved from exp_88's 0.5116)
  H3: Selection mechanism coherence improved (less negative)
  H4: GBC pass_rate >= 0.3 (improved from exp_88's 0.1875)
"""
...
class NarrativeMomentumConnectorV2(NarrativeConnector):
    """V2: configurable momentum bonus."""
...
def run_single_seed(N0=72, steps=1600, seed=142, sample_interval=10, gbc_soft_nudge=0.2):
...
def main():
    print("=" * 70)
    print("exp_89: GBC Soft Nudge (bonus=0.3, threshold=0.5, nudge=0.2)")
    print("=" * 70)

    seeds = [242, 442, 642, 742]
    all_results = []

    for seed in seeds:
        print(f"\n--- Seed {seed} ---", flush=True)
        result = run_single_seed(N0=72, steps=1600, seed=seed, sample_interval=10, gbc_soft_nudge=0.2)
        ...
        print(f"  Selection coherence: {result['gbc']['mechanism_mean_coherence'].get('selection', 'N/A')}", flush=True)
    ...
    # Per-seed comparison with exp_88
    exp_88_civ = {242: 6, 442: 4, 642: 3, 742: 7}
    exp_88_coh = {242: 0.5001, 442: 0.5183, 642: 0.4961, 742: 0.5319}
    print(f"\n  {'Seed':>6} | {'CIV':>5} | {'exp_88':>7} | {'Delta':>6} | {'GBC_coh':>8} | {'exp_88_coh':>10} | Status", flush=True)
    print(f"  {'-'*6}-+-{'-'*5}-+-{'-'*7}-+-{'-'*6}-+-{'-'*8}-+-{'-'*10}-+-{'-'*6}", flush=True)
    for r in all_results:
        status = "PASS" if r['civilization_steps'] >= 5 else "FAIL"
        delta = r['civilization_steps'] - exp_88_civ.get(r['seed'], 0)
        delta_str = f"+{delta}" if delta >= 0 else str(delta)
        coh_delta = r['gbc']['coherence_mean'] - exp_88_coh.get(r['seed'], 0)
        print(f"  {r['seed']:>6} | {r['civilization_steps']:>5} | {exp_88_civ.get(r['seed'], '?'):>7} | {delta_str:>6} | {r['gbc']['coherence_mean']:>8.4f} | {exp_88_coh.get(r['seed'], 0):>10.4f} | {status}", flush=True)

    h1 = civ_mean >= 5.0
    h2 = float(np.mean(gbc_coh_values)) >= 0.55
    h3 = float(np.mean(gbc_pass_values)) >= 0.3

    # Selection coherence improvement
    sel_cohs = [r['gbc']['mechanism_mean_coherence'].get('selection', 0) for r in all_results]
    exp_88_sel = {242: -0.1204, 442: 0.0675, 642: 0.199, 742: -0.3549}
    print(f"\n  Selection coherence:", flush=True)
    for r in all_results:
        s = r['gbc']['mechanism_mean_coherence'].get('selection', 0)
        s88 = exp_88_sel.get(r['seed'], 0)
        print(f"    seed={r['seed']}: exp_88={s88:+.4f} -> exp_89={s:+.4f} ({'+'if s>s88 else ''}{s-s88:.4f})", flush=True)
    ...
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                f'exp_89_results_{ts}.json')
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump({
            'experiment': 'exp_89_gbc_soft_nudge',
            'timestamp': ts,
            'seeds': seeds,
            'n_seeds': len(seeds),
            'config': {
