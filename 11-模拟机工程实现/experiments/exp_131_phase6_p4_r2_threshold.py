"""
experiments/exp_131_phase6_p4_r2_threshold.py

Phase 6 P4-A: R2 Threshold Tuning — Lower threshold from 0.85 to 0.80

Purpose:
  exp_130 showed R2 dormant at N0=48 even with 5000 steps, no booster.
  Root cause analysis: 6/8 seeds hit NSI >= 0.85, but cycle_count > 5
  condition blocks R2 because all cycles cluster in first ~200 steps.

  This experiment tests TWO changes:
  1. Lower r2_threshold_nsi: 0.85 -> 0.80
  2. Lower r2_cooldown: 200 -> 100 (allow R2 to trigger sooner after cycles)

  Also tests at 3000 steps (between 2000 and 5000) to find minimum
  steps needed if R2 does activate.

Hypotheses:
  H66 (R2 with lower threshold): >=4/8 seeds trigger R2 with threshold=0.80
  H67 (R2 with shorter cooldown): R2 activates when cooldown=100 vs 200
  H68 (steps sensitivity): 3000 steps sufficient if threshold fix works

Baseline: H1-H8 must still pass (threshold tuning should not destabilize)
"""
...
def run_single_seed(N0, steps, seed, sample_interval, gbc_soft_nudge,
                    r2_threshold, r2_cooldown, csc_config=None,
                    tracking_collector=None):
    """Run a single seed with CSC+NSE+NRC (NO booster)."""
    ...
    # *** KEY CHANGE: Lower R2 threshold and cooldown ***
    nrc = NarrativeRecursiveClosure(
        ...
        r2_threshold_nsi=r2_threshold, r2_cooldown=r2_cooldown, verbose=True)
    ...
    print(f"    [seed={seed}] Running {steps} steps (threshold={r2_threshold}, cooldown={r2_cooldown})...", flush=True)
    start = time.time()
    ...
    return {
        ...
        'r2_cooldown_used': r2_cooldown,
    }


def evaluate_hypotheses(results, label="unknown"):
    """Evaluate H1-H8 + H66/H67/H68."""
    if not results:
    ...
    # H66: R2 with lower threshold
    all_r2_counts = [r.get('nrc_metrics', {}).get('n_r2_events', 0) for r in results]
    h66 = sum(1 for cnt in all_r2_counts if cnt >= 1) >= 4
    h66_total_r2 = int(np.sum(all_r2_counts))

    # H67: Any R2 activation (relaxed: >=2/8 seeds)
    h67 = sum(1 for cnt in all_r2_counts if cnt >= 1) >= 2

    # H68: Steps sensitivity (if R2 activates, check cycle timing)
    cycles_per_1k = [r.get('nrc_metrics', {}).get('h64_cycles_per_1000', 0.0)
                     for r in results]
    h68 = float(np.mean(cycles_per_1k)) > 0.5  # some cycling happening
    ...
    return {
        ...
        'H66_r2_lower_threshold': {
            'value': '%d/8 seeds with R2 (total=%d)' % (sum(1 for v in all_r2_counts if v >= 1), h66_total_r2),
            'threshold': '>=4 seeds', 'pass': h66},
        'H67_r2_any_activation': {
            'value': '%d/8 seeds with R2 (total=%d)' % (sum(1 for v in all_r2_counts if v >= 1), h66_total_r2),
            'threshold': '>=2 seeds', 'pass': h67},
        'H68_steps_sensitivity': {
            'value': 'mean_cycles/1k=%.2f' % float(np.mean(cycles_per_1k)),
            'threshold': '>0.5', 'pass': h68},
        'nrc_aggregate': {
        ...
        'p6_summary': {
            'n_pass': sum([h66, h67, h68]),
            'all_pass': h66 and h67 and h68,
            'failed': [n for n, v in [('H66', h66), ('H67', h67), ('H68', h68)] if not v],
        },
    }


# --- Experiment matrix ---
# Config A: threshold=0.80, cooldown=100 (most permissive)
# Config B: threshold=0.80, cooldown=200 (only threshold change)
# Config C: threshold=0.75, cooldown=100 (most permissive overall)

CONFIGS = [
    {'label': 'th0.80_cd100', 'r2_threshold': 0.80, 'r2_cooldown': 100},
    {'label': 'th0.80_cd200', 'r2_threshold': 0.80, 'r2_cooldown': 200},
    {'label': 'th0.75_cd100', 'r2_threshold': 0.75, 'r2_cooldown': 100},
]
...
def main():
    print("=" * 70)
    print("exp_131: Phase 6 P4-A — R2 Threshold Tuning")
    print("  Architecture: CSC+NSE+NRC (NO Booster)")
    print("  N0=%d  Seeds: %d  Steps: %d" % (N0, len(SEEDS), N_STEPS))
    print("  Configs: %d" % len(CONFIGS))
    for cfg in CONFIGS:
        print("    %s: threshold=%.2f, cooldown=%d" % (cfg['label'], cfg['r2_threshold'], cfg['r2_cooldown']))
    print("  Hypotheses: H66 (>=4/8 R2) H67 (>=2/8 R2) H68 (cycles/1k)")
    ...
    for cfg in CONFIGS:
        print("\n" + "-" * 60)
        print("CONFIG: %s (threshold=%.2f, cooldown=%d)" % (cfg['label'], cfg['r2_threshold'], cfg['r2_cooldown']))
        print("-" * 60)
        ...
        for seed in SEEDS:
            try:
                ...
                config_results.append({
                    ...
                    'r2_threshold_used': cfg['r2_threshold'],
                    'r2_cooldown_used': cfg['r2_cooldown'],
                })
    ...
    # --- Summary ---
    print("\n" + "=" * 70)
    print("PHASE 6 P4-A RESULTS — R2 Threshold Tuning Summary")
    print("=" * 70)
    ...
    for cfg_label, data in all_config_results.items():
        ...
        print(f"    H66 (>=4/8 R2): {'PASS' if h.get('H66_r2_lower_threshold', {}).get('pass') else 'FAIL'} — {h.get('H66_r2_lower_threshold', {}).get('value', '?')}")
        print(f"    H67 (>=2/8 R2): {'PASS' if h.get('H67_r2_any_activation', {}).get('pass') else 'FAIL'} — {h.get('H67_r2_any_activation', {}).get('value', '?')}")
        print(f"    H68 (cycles/1k): {'PASS' if h.get('H68_steps_sensitivity', {}).get('pass') else 'FAIL'} — {h.get('H68_steps_sensitivity', {}).get('value', '?')}")
    ...
    if best_config:
        ...
        for r in all_config_results[best_config]['results']:
            ...
            print(f"  seed={r.get('seed', '?'):>3}: "
                  ...
                  f"conv={nrc_m.get('h63_converges', False)} {error_str}")

    # --- Save results ---
    timestamp = datetime.now().strftime("%Y%m%d_%H%M")
    results_file = os.path.join(
        PROJECT_ROOT, 'experiments',
        'exp_131_phase6_p4_r2_threshold_%s.json' % timestamp)
    save_data = {
        'experiment': 'exp_131_phase6_p4_r2_threshold',
        'datetime': datetime.now().isoformat(),
        'N0': N0, 'n_steps': N_STEPS,
        'configs': {cfg_label: {'r2_threshold': data['results'][0].get('r2_threshold_used', 0) if data['results'] else 0,
                                 'r2_cooldown': data['results'][0].get('r2_cooldown_used', 0) if data['results'] else 0}
                    for cfg_label, data in all_config_results.items()},
