"""experiments/exp_78_narrative_activation.py

Phase 3 实验八：叙事递归激活验证（narrative fix + ODI patch）

Purpose:
  验证 commit 4eab9b2 (narrative direction vector fix) 和 ODI 实参传递修复后，
  叙事递归算子是否能在长演化中真正激活并产生非零偏置修正。
...
def run_experiment(N0=72, steps=800, seed=42, sample_interval=5):
    ...
    print(f"[exp_78] N0={N0}, steps={steps}, seed={seed}, sample_interval={sample_interval}")
    start = time.time()
    result = evolver.run()
...
def main():
    ...
    output_file = os.path.join(
        ...
        f'exp_78_results_{timestamp}.json'
    )

    print(f"{'='*60}")
    print(f"exp_78: Narrative Activation Verification")
    print(f"Testing: direction vector fix + ODI passthrough")
    print(f"Output: {output_file}")
    print(f"{'='*60}")

    run_data = run_experiment(N0=72, steps=800, seed=42, sample_interval=5)
    diag = run_data['diagnostic']

    # Print summary
    print(f"\n{'='*60}")
    print(f"exp_78 Results (seed={diag['seed']}):")
    ...
    # Save
    summary = {
        'experiment': 'exp_78_narrative_activation',
        'timestamp': timestamp,
        'description': 'Narrative activation verification after direction vector + ODI fix',
        'fixes_tested': [
