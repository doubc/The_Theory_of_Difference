"""experiments/exp_104_p1c_fixes.py

Phase 4 Experiment 14: P1-C Fixes — INSTITUTIONAL-Driven TopDown & CIV Relaxation

Purpose: Fix H5/H6 (CIV over-suppression) and H8 (TopDown not activating) from exp_103.

exp_103 results: 5/8 pass
  H5 FAIL: CIV mean=2.5 (below [3,15]) — limiter too tight (max_rate=0.05, cooldown=5)
  H6 FAIL: CIV min=1 (below 3) — same root cause
  H8 FAIL: TopDown active seeds=0 — CIV layer too sparse to drive TopDown

P1-C fixes:
  1. Relax CIVRateLimiter: max_rate 0.05->0.08, cooldown 5->8
     - P1-B over-corrected; exp_103 CIV mean=2.5 is too low
     - Target: CIV mean in [3, 15], min >= 3
  2. TopDown threshold: 0.15 -> 0.10
     - Further lowered to allow INSTITUTIONAL layer to drive TopDown
     - INSTITUTIONAL has more activity than CIV (rare-event layer)
  3. CIV stability gating: only count when events > 10, else default 0.3
     - Prevents noisy CIV stability from interfering with TopDown
     - CIV layer is inherently sparse (1-4 events/seed), stability metric unreliable
  4. INSTITUTIONAL ILP fallback threshold raised: stab < 0.15 (was 0.1)
     - Gives INSTITUTIONAL layer more opportunity to boost stability via ILP
     - ILP's internal stability_score is more reliable than raw hierarchy signal

Config: Same as exp_103 (N0=72, steps=1600, 8 seeds)
    CSC: ON (topdown_stability_threshold=0.10)
    GBC: ON (random direction init, soft nudge=0.2)
    NSE: ON (threshold=0.02, odi=0.4, msi=0.3, civ=0.0, gbc=0.1, max_tp=25)
        Same moderated non-triviality as P1-B
    AMC: ON, ILP: ON
    CIVRateLimiterV2P1C: ON (window=50, max_rate=0.08, cooldown=8, min_guarantee=3)
...
EXP104_CSC_CONFIG = {
    ...
    'topdown_stability_threshold': 0.10,  # P1-C: 0.15 -> 0.10 (INSTITUTIONAL-driven)
...
class CIVRateLimiterV2P1C(CIVRateLimiter):
    """CIVRateLimiter V2 P1-C — relaxed rate limiting

    P1-B over-corrected with max_rate=0.05, cooldown=5.
    P1-C relaxes to max_rate=0.08, cooldown=8 to allow CIV mean recovery to [3, 15].
    """

    def __init__(self, window_size=50, max_civ_rate=0.08, cooldown_steps=8,
                 min_civ_guarantee=3):
...
class MomentumNarrativeOperatorV4P1C(NarrativeRecursionOperator):
    """V4 P1-C: CIVRateLimiterV2P1C with relaxed limits"""
    ...
    def __init__(self, bias_dimension=128, filter_magnitude_threshold=0.02,
        ...
        self.civ_rate_limiter = CIVRateLimiterV2P1C(
            window_size=50, max_civ_rate=0.08, cooldown_steps=8,
            min_civ_guarantee=3
        )
...
def run_single_seed(N0=72, steps=1600, seed=142, sample_interval=10,
    ...
    narrative = MomentumNarrativeOperatorV4P1C(
...
def main():
    ...
    print(f"exp_104: P1-C Fixes — INSTITUTIONAL-Driven TopDown & CIV Relaxation")
    print(f"  N0={N0}, steps={STEPS}, {len(SEEDS)} seeds")
    print(f"  CSC=ON (topdown_threshold=0.10), GBC=ON (random dir), AMC=ON, ILP=ON")
    print(f"  NSE=ON: max_turning_points=25, moderated non-triviality")
    print(f"  CIVRateLimiterV2P1C: max_rate=0.08 (was 0.05), cooldown=8 (was 5)")
    print(f"  Fixes: relaxed limiter, TopDown thresh 0.10, CIV stability gating, ILP fallback threshold 0.15")
    print()
    ...
    for seed in SEEDS:
        result = run_single_seed(
            ...
            gbc_soft_nudge=GBC_SOFT_NUDGE,
            use_csc=True, csc_config=EXP104_CSC_CONFIG,
            use_nse=True, use_amc=True, use_ilp=True,
        )
    ...
    # Comparison with exp_103
    print()
    print("COMPARISON WITH exp_103")
    print("-" * 60)
    print(f"  {'Seed':>6} | {'e103 NSI':>10} | {'e104 NSI':>10} | "
          f"{'e103 stab':>10} | {'e104 stab':>10} | "
          f"{'e103 civ':>9} | {'e104 civ':>9} | "
          f"{'e103 td':>8} | {'e104 td':>8}")
    exp_103_nsi = {42: 0.769, 142: 0.642, 242: 0.710, 342: 0.598,
                   442: 0.795, 542: 0.675, 642: 0.622, 742: 0.734}
    exp_103_stab = {42: 0.633, 142: 0.630, 242: 0.636, 342: 0.633,
                    442: 0.630, 542: 0.630, 642: 0.633, 742: 0.630}
    exp_103_civ = {42: 3, 142: 2, 242: 3, 342: 1, 442: 2, 542: 3, 642: 4, 742: 2}
    exp_103_td = {42: 0, 142: 0, 242: 0, 342: 0, 442: 0, 542: 0, 642: 0, 742: 0}
    for r in all_results:
        s = r['seed']
        print(f"  {s:>6} | {exp_103_nsi.get(s, 0):>10.4f} | {r['nse_nsi_max']:>10.4f} | "
              f"{exp_103_stab.get(s, 0):>10.4f} | {r['nse_stability_mean']:>10.4f} | "
              f"{exp_103_civ.get(s, 0):>9} | {r['civ_count']:>9} | "
              f"{exp_103_td.get(s, 0):>8} | {r['topdown_max_active']:>8}")

    # Save results
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    output = {
        'experiment': 'exp_104_p1c_fixes',
        'timestamp': timestamp,
        'config': {
            ...
            'amc': 'ON', 'ilp': 'ON',
            'civ_rate_limiter_v2_p1c': 'ON (max_rate=0.08, cooldown=8, min_guarantee=3)',
            'fixes_from_exp_103': [
                'CIVRateLimiter: max_rate 0.05->0.08, cooldown 5->8 (relax over-suppression)',
                'TopDown threshold: 0.15->0.10 (enable INSTITUTIONAL-driven activation)',
                'CIV stability gating: only count when events > 10, else default 0.3',
                'INSTITUTIONAL ILP fallback threshold: 0.1->0.15 (more aggressive boost)',
            ],
        },
    ...
    output_path = os.path.join(output_dir, f'exp_104_results_{timestamp}.json')
