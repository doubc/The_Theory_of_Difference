"""
exp_215_p4_narrative_recursion.py — Phase 23 P4 叙事递归实验

测试 H23-4 假设：叙事递归突破循环陷阱 + 能量注入改变拓扑 + 差异化选择 + 可持续涌现

实验设计:
- G3: 封闭系统/自指闭环 (baseline, self_encapsulate=True)
- G4: 开放系统/自指闭环/无叙事递归 (能量+自指, energy truncates)
- G5: 开放系统/自指闭环/叙事递归 (full V1.7 implementation)

核心假设:
- H23-4a: 叙事递归突破循环陷阱 (G5 depth > G3 depth)
- H23-4b: 能量注入改变可能性空间拓扑 (G5 flux > G3 flux)
- H23-4c: 差异化选择避免奖励 hacking (G5 non-repetition > G3)
- H23-4d: 可持续涌现 (G5 depth >= 3, flux > 0)
...
class NarrativeRecursiveWorld(RecursiveWorld):
    ...
    def _m9_seal_and_create_next(self, current_layer, verbose: bool = False) -> bool:
        ...
        if self.narrative_loop is None:
            ...
            if energy_field is None:
                # 创建默认能量管理器
                from diffsim.energy_v2 import EnergyManager
        ...
        # 根据叙事递归结果调整 f_next
        self._adjust_next_field(f_next, spiral_history)
    ...
    def _adjust_next_field(self, f_next, spiral_history: List[NarrativeState]):
        """根据叙事递归结果调整下一层的字段
...
def main():
    """主实验"""
    print("=" * 60)
    print("Phase 23 P4: Narrative Recursion Experiment (exp_215)")
    print("Testing H23-4 hypotheses")
    print("=" * 60)
    ...
    experiments = {
        ...
        'G4': {
            **base_config,
            'use_energy': True,
            'energy_budget': 500.0,  # 增加预算确保 m9 能执行
            'injection_rate': 2.0,  # 增加注入率
            'use_entropy': False,
            'self_encapsulate': True,
        },
    ...
    # 保存结果
    output = {
        'experiment': 'exp_215_p4_narrative_recursion',
    ...
    output_path = os.path.join(os.path.dirname(__file__), '../results/exp_215_p4_narrative.json')
