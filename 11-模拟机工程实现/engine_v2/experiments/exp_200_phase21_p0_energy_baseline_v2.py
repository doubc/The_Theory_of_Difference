"""exp_200_phase21_p0_energy_baseline_v2.py

Phase 21 P0: 能量流基线实验 (v2 — 修复 baseline bug)

修复:
- baseline: energy_config=None => Layer.energy = None (真正无能量流)
- 增加更大预算配置: budget_200, budget_500
- 修复 is_irreversible 检测 (abs(p) > 1e-10)
...
- with_energy: 有能量流 + 注入 (budget=100)
- low_budget: 低初始预算 (30.0)
- high_decay: 高衰减率 (0.05)
- budget_200: 中高预算 (200.0)
- budget_500: 高预算 (500.0)
...
def run_single(cfg_name: str, seed: int, N0: int = 48) -> dict:
    ...
    if cfg_name == "baseline":
        ...
        entropy_cfg = EntropyConfig()
    elif cfg_name == "budget_200":
        energy_cfg = EnergyConfig(initial_budget=200.0, decay_rate=0.01, injection_rate=0.5)
        entropy_cfg = EntropyConfig()
    elif cfg_name == "budget_500":
        energy_cfg = EnergyConfig(initial_budget=500.0, decay_rate=0.01, injection_rate=0.5)
        entropy_cfg = EntropyConfig()
    else:
    ...
    report = world.run(max_layers=8, verbose=False)
...
def evaluate_hypotheses(results: list) -> dict:
    ...
    print("\n  H21-P0a (dead order layers):")
    for c in by_cfg:
        n_dead = sum(r["dead_order_layers"] for r in by_cfg[c])
        n_total = sum(max(len(r["layer_energy"]), 1) for r in by_cfg[c])
        print(f"    {c}: {n_dead}/{n_total} layers in dead order")
    eval_out["H21-P0a"] = "see printed stats"

    # H21-P0b: 有能量注入时 flux 维持更久
    print("\n  H21-P0b (mean L1 flux):")
    for c in configs:
        fluxes = []
        for r in by_cfg[c]:
            if r["report"] and len(r["report"]) > 1:
                fluxes.append(r["report"][1].get("autonomous_flux", 0.0))
        if fluxes:
            print(f"    {c}: mean L1 flux = {np.mean(fluxes):.4f}")
    eval_out["H21-P0b"] = "see printed stats"

    # H21-P0c: 涌现深度与初始能量预算正相关
    print("\n  H21-P0c (mean depth):")
    for c in configs:
        depths = [r["emergence_depth"] for r in by_cfg[c]]
        print(f"    {c}: mean depth = {np.mean(depths):.2f}")
    eval_out["H21-P0c"] = "see printed stats"

    # H21-P0d: 熵产生 != 0 (不可逆)
    print("\n  H21-P0d (irreversible):")
    for c in configs:
...
def main():
    configs = ["baseline", "with_energy", "low_budget", "high_decay", "budget_200", "budget_500"]
    seeds = range(8)  # 0-7
    N0 = 48
    ...
    for cfg_name in configs:
        for seed in seeds:
            ...
            try:
                ...
                flux = result["report"][1].get("autonomous_flux", 0.0) if len(result["report"]) > 1 else 0.0
                print(f"    depth={depth} L1_flux={flux:.4f}")
    ...
    out_path = out_dir / "exp_200_p0_energy_baseline_v2.json"
